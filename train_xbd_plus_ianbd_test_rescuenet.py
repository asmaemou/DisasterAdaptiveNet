...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser("Train on xBD + IAN-BD, test on RescueNet-xBD")
    ...
    parser.add_argument("--output-dir", type=str, default="output/xbd_plus_ianbd_to_rescuenet")
...
def main() -> None:
    ...
    train_loader = make_balanced_concat_loader([xbd_train, ian_train], args.batch_size, args.num_workers)
    val_loader = make_eval_loader(ConcatDataset([xbd_val, ian_val]), args.batch_size, args.num_workers)
    target_test_loader = make_eval_loader(target_test, args.batch_size, args.num_workers)

    loc_pos_weight, dmg_class_weights = aggregate_counts([xbd_train, ian_train])
