    ...
    def __init__(self, root: str | Path, split: str, image_size: int, training: bool, conditioning_id: int = 0, source_name: str = ""):
        ...
        if not self.images_dir.exists() or not self.targets_dir.exists():
            raise FileNotFoundError(
                f"Expected xBD-like directories under {self.split_root}. "
                f"Need {self.images_dir} and {self.targets_dir}."
            )
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        "Practical MADA-style training with DisasterAdaptiveNet for IDA-BD -> Hurricane-IRMA"
    )
    p.add_argument("--ida-root", type=str, default="/homes/j244s673/documents/wsu/phd/idabd_real_disasteradaptivenet")
    p.add_argument(
        "--irma-root",
        type=str,
        default="/homes/j244s673/documents/wsu/phd/HURRICANE-IRMA_disasteradaptivenet",
        help="xBD-like standardized IRMA root: split/images/*_pre_disaster.* + *_post_disaster.* and split/targets/*_target.png",
    )
    p.add_argument("--translated-ida-root", type=str, default="", help="Optional CycleGAN-translated IDA root in the same xBD-like format")
    p.add_argument("--ida-train-split", type=str, default="train")
    p.add_argument("--ida-val-split", type=str, default="val")
    p.add_argument("--irma-train-split", type=str, default="train")
    p.add_argument("--irma-val-split", type=str, default="val")
    p.add_argument("--irma-test-split", type=str, default="test")
    p.add_argument("--output-dir", type=str, default="output/disasteradaptivenet_mada_ida_to_irma")
...
def get_root(args: argparse.Namespace, name: str, translated: bool = False) -> str:
    if name == "ida":
        return args.translated_ida_root if translated and args.translated_ida_root else args.ida_root
    if name == "irma":
        return args.irma_root
    raise KeyError(name)


def get_split(args: argparse.Namespace, name: str, which: str) -> str:
    mapping = {
        "ida": {"train": args.ida_train_split, "val": args.ida_val_split},
        "irma": {"train": args.irma_train_split, "val": args.irma_val_split, "test": args.irma_test_split},
    }
    return mapping[name][which]


def build_labeled_dataset(args: argparse.Namespace, name: str, split: str, training: bool, use_translated: bool) -> Dataset:
    return XBDLikeLabeledDataset(get_root(args, name, translated=use_translated), split, args.img_size, training, args.conditioning_id, name)


def build_unlabeled_dataset(args: argparse.Namespace, name: str, split: str, training: bool) -> Dataset:
    return XBDLikeUnlabeledDataset(get_root(args, name, translated=False), split, args.img_size, training, args.conditioning_id, name)
...
def evaluate_source_validation(model: nn.Module, loader: DataLoader, loc_criterion: BCEDiceLoss, dmg_criterion: nn.Module, device: torch.device, args: argparse.Namespace) -> Dict[str, float]:
    ...
    return {
        ...
        "loc_dice_loss": loc_dice_meter.avg,
        "loc_dice": loc_dice_score_meter.avg,
...
def evaluate_target_test_f1(model: nn.Module, loader: DataLoader, device: torch.device, loc_threshold: float) -> Dict[str, object]:
    ...
    for batch in iterator:
        ...
        dmg_pred = torch.argmax(logits[:, 1:5], dim=1) + 1
...
def write_scores(results: Dict[str, object], output_dir: Path, target_name: str) -> None:
    ...
    with open(scores_dir / f"scores_{target_name}_test.json", "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)
    with open(scores_dir / f"scores_{target_name}_test.txt", "w", encoding="utf-8") as f:
...
def main() -> None:
    ...
    with open(output_dir / "mada_config.json", "w", encoding="utf-8") as f:
        json.dump({"source": "ida", "target": "irma", **vars(args)}, f, indent=2)

    device = torch.device(args.device if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}", flush=True)
    print("Source: ida", flush=True)
    print("Target: irma", flush=True)

    source_train = build_labeled_dataset(args, "ida", get_split(args, "ida", "train"), True, use_translated=True)
    source_val = build_labeled_dataset(args, "ida", get_split(args, "ida", "val"), False, use_translated=True)

    target_unlabeled_parts = [
        build_unlabeled_dataset(args, "irma", get_split(args, "irma", "train"), True),
        build_unlabeled_dataset(args, "irma", get_split(args, "irma", "val"), True),
    ]
    target_test_dataset = build_labeled_dataset(args, "irma", get_split(args, "irma", "test"), False, use_translated=False)

    src_train_loader = DataLoader(source_train, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True, drop_last=True)
    src_val_loader = DataLoader(source_val, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True, drop_last=False)
    ...
    print(f"Source train [ida]: {len(source_train)}", flush=True)
    print(f"Source val   [ida]: {len(source_val)}", flush=True)
    print(f"Target unlabeled total [irma]: {sum(len(x) for x in target_unlabeled_parts)}", flush=True)
    print(f"Target test [irma]: {len(target_test_dataset)}", flush=True)

    model = make_model(device)
    domain_disc = ConditionalDomainClassifier(feature_dim=10, num_domains=2).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, src_batch in enumerate(iterator, start=1):
            ...
            src_domain = torch.zeros((src_img.size(0),), device=device, dtype=torch.long)

            tgt_img = tgt_batch["img"].to(device, non_blocking=True)
            tgt_cond = tgt_batch["cond_id"].to(device, non_blocking=True)
            tgt_domain = torch.ones((tgt_img.size(0),), device=device, dtype=torch.long)
    ...
    print("Evaluating best checkpoint on target test split: irma", flush=True)
    ...
    write_scores(target_results, output_dir, "irma")
