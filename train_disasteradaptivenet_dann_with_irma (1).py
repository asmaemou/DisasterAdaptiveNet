...
class DomainDiscriminator(nn.Module):
    ...
    def forward(self, pooled_logits: torch.Tensor, grl_lambda: float) -> torch.Tensor:
        x = grad_reverse(pooled_logits, grl_lambda)
        return self.net(x).squeeze(1)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser("DisasterAdaptiveNet + DANN with xBD/IDA/IAN/IRMA")
...
def get_test_split(args: argparse.Namespace, name: str) -> str:
    return {
        ...
        "irma": args.irma_test_split,
    }[name]


def build_labeled_dataset(args: argparse.Namespace, name: str, split: str, training: bool) -> Dataset:
    return XBDStyleLabeledDataset(get_dataset_root(args, name), split, args.img_size, training, name, args.conditioning_id)


def build_unlabeled_dataset(args: argparse.Namespace, name: str, split: str, training: bool) -> Dataset:
    return XBDStyleUnlabeledDataset(get_dataset_root(args, name), split, args.img_size, training, name, args.conditioning_id)
...
def pool_logits_for_domain(logits: torch.Tensor) -> torch.Tensor:
...
def main() -> None:
    ...
    src_train = build_labeled_dataset(args, args.source_dataset, get_train_split(args, args.source_dataset), True)
    src_val = build_labeled_dataset(args, args.source_dataset, get_val_split(args, args.source_dataset), False)

    tgt_train_u = build_unlabeled_dataset(args, args.target_dataset, get_train_split(args, args.target_dataset), True)
    tgt_val_u = build_unlabeled_dataset(args, args.target_dataset, get_val_split(args, args.target_dataset), True)
    tgt_test = build_labeled_dataset(args, args.target_dataset, get_test_split(args, args.target_dataset), False)
    ...
    src_val_loader = make_eval_loader(src_val, args.batch_size, args.num_workers)
    tgt_u_loader = make_unlabeled_loader(ConcatDataset([tgt_train_u, tgt_val_u]), args.batch_size, args.num_workers)
    tgt_test_loader = make_eval_loader(tgt_test, args.batch_size, args.num_workers)

    loc_pos_weight, dmg_class_weights = aggregate_counts([src_train])
    print(f"{args.source_dataset} train samples: {len(src_train)} | val samples: {len(src_val)}", flush=True)
    print(f"{args.target_dataset} unlabeled train samples: {len(tgt_train_u)} | unlabeled val samples: {len(tgt_val_u)}", flush=True)
    print(f"{args.target_dataset} test samples: {len(tgt_test)}", flush=True)
    ...
    domain_disc = DomainDiscriminator(in_dim=5).to(device)
    ...
    target_iter = cycle_loader(tgt_u_loader)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, src_batch in enumerate(iterator, start=1):
            ...
            if USE_TORCH_AMP:
                with autocast(device_type=device.type, enabled=args.amp and device.type == "cuda"):
                    ...
                    src_dom_logits = domain_disc(pool_logits_for_domain(src_logits), grl_lambda)
                    tgt_dom_logits = domain_disc(pool_logits_for_domain(tgt_logits), grl_lambda)
            ...
            if use_tqdm:
                iterator.set_postfix(loss=f"{total_meter.avg:.4f}", sup=f"{sup_meter.avg:.4f}", dom=f"{dom_meter.avg:.4f}", domacc=f"{dom_acc_meter.avg:.4f}", grl=f"{grl_lambda:.3f}")
            elif step % 20 == 0 or step == num_steps:
                print(
                    f"Epoch {epoch}/{args.epochs} | Step {step}/{num_steps} | loss={total_meter.avg:.4f} | sup={sup_meter.avg:.4f} | dom={dom_meter.avg:.4f} | domacc={dom_acc_meter.avg:.4f} | grl={grl_lambda:.3f}",
                    flush=True,
                )
        ...
        print(
            f"Epoch {epoch:03d} | train_total={row['train_total_loss']:.4f} | train_domain={row['train_domain_loss']:.4f} | src_val_score={row['src_val_score']:.4f} | src_val_loc_dice={row['src_val_loc_dice']:.4f} | src_val_dmg_macro_f1={row['src_val_dmg_macro_f1']:.4f}",
            flush=True,
        )
