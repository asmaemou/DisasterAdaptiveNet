...
class XView2DADataset(Dataset):
    def __init__(self, split_dir: str, mode: str = "train", crop_size: int = 512, stride: int = 256, with_labels: bool = True):
        ...
        if not self.image_dir.exists():
            raise FileNotFoundError(f"Missing images under {self.split_dir}")

        self.pre_imgs = sorted(self.image_dir.glob("*_pre_disaster.png"))
        if not self.pre_imgs:
            self.pre_imgs = sorted(self.image_dir.glob("*_pre_*.png"))
        if not self.pre_imgs:
            raise FileNotFoundError(f"No pre-disaster images found in {self.image_dir}")

        self.samples: List[Dict[str, Path]] = []
        for pre_img in self.pre_imgs:
            post_name = pre_img.name.replace("_pre_disaster.png", "_post_disaster.png")
            if post_name == pre_img.name:
                post_name = pre_img.name.replace("_pre_", "_post_")
            post_img = self.image_dir / post_name
            if not post_img.exists():
                continue

            sample = {"pre_img": pre_img, "post_img": post_img}
            if with_labels:
                pre_tgt = self.target_dir / pre_img.name.replace(".png", "_target.png")
                post_tgt = self.target_dir / post_img.name.replace(".png", "_target.png")
                if not pre_tgt.exists() or not post_tgt.exists():
                    continue
                sample["pre_mask"] = pre_tgt
                sample["post_mask"] = post_tgt
            self.samples.append(sample)

        if not self.samples:
            raise RuntimeError(f"No valid samples found in {self.split_dir}")

        print(f"[dataset] split={self.split_dir.name} mode={self.mode} labels={self.with_labels} samples={len(self.samples)}")
    ...
    def __getitem__(self, idx):
        ...
        out = {"pre": pre_t, "post": post_t, "image_filename": name}
        if self.with_labels:
            out["loc_mask"] = torch.from_numpy((pre_mask > 0).astype(np.uint8))
            out["dam_mask"] = torch.from_numpy(post_mask.astype(np.uint8))
        return out
...
class TransformerDANNDamage(nn.Module):
    ...
    def encode_fused(self, pre: torch.Tensor, post: torch.Tensor) -> torch.Tensor:
    ...
    def forward(self, pre: torch.Tensor, post: torch.Tensor, lambda_grl: float = 0.0):
        fused = self.encode_fused(pre, post)
        loc_logits = self.loc_head(fused)
        dam_logits = self.dam_head(fused)
        pooled = F.adaptive_avg_pool2d(fused, 1).flatten(1)
        dom_logits = self.domain_disc(grad_reverse(pooled, lambda_grl))
        return loc_logits, dam_logits, dom_logits
...
def evaluate(model: nn.Module, loader: DataLoader, device: torch.device, loc_threshold: float = 0.5) -> Dict[str, float]:
    ...
    for batch in loader:
        ...
        loc_logits, dam_logits, _ = model(pre, post, lambda_grl=0.0)
...
def main():
    ap = argparse.ArgumentParser(description="Transformer feature extractor + domain adaptation + classifier for damage assessment")
    ...
    val_root = Path(args.val_root) if args.val_root else target_root
    eval_root = Path(args.eval_root) if args.eval_root else target_root

    source_dir = source_root / args.source_split
    target_dir = target_root / args.target_split
    val_dir = val_root / args.val_split
    eval_dir = eval_root / args.eval_split

    print(f"[paths] source_train={source_dir}")
    print(f"[paths] target_train={target_dir}")
    print(f"[paths] val={val_dir}")
    print(f"[paths] eval={eval_dir}")

    src_train_ds = XView2DADataset(str(source_dir), mode="train", crop_size=args.crop_size, stride=args.stride, with_labels=True)
    tgt_train_ds = XView2DADataset(str(target_dir), mode="train", crop_size=args.crop_size, stride=args.stride, with_labels=False)
    val_ds = XView2DADataset(str(val_dir), mode="eval", with_labels=True)
    eval_ds = XView2DADataset(str(eval_dir), mode="eval", with_labels=True)

    src_loader = DataLoader(src_train_ds, batch_size=args.batch_size, shuffle=True, num_workers=4, pin_memory=True)
    tgt_loader = DataLoader(tgt_train_ds, batch_size=args.batch_size, shuffle=True, num_workers=4, pin_memory=True)
    ...
    model = TransformerDANNDamage().to(device)
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    domain_criterion = nn.BCEWithLogitsLoss()

    best_val = -1.0
    total_steps = args.epochs * max(1, len(src_loader))
    global_step = 0
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for src_batch in src_loader:
            ...
            lambda_grl = args.lambda_domain * (2.0 / (1.0 + math.exp(-10.0 * p)) - 1.0)

            src_pre = src_batch["pre"].to(device)
            src_post = src_batch["post"].to(device)
            src_loc_gt = src_batch["loc_mask"].to(device).float().unsqueeze(1)
            src_dam_gt = src_batch["dam_mask"].to(device).long()

            tgt_pre = tgt_batch["pre"].to(device)
            tgt_post = tgt_batch["post"].to(device)

            src_loc_logits, src_dam_logits, src_dom_logits = model(src_pre, src_post, lambda_grl=lambda_grl)
            _, _, tgt_dom_logits = model(tgt_pre, tgt_post, lambda_grl=lambda_grl)

            if src_loc_logits.shape[-2:] != src_loc_gt.shape[-2:]:
                src_loc_logits = F.interpolate(src_loc_logits, size=src_loc_gt.shape[-2:], mode="bilinear", align_corners=False)
            if src_dam_logits.shape[-2:] != src_dam_gt.shape[-2:]:
                src_dam_logits = F.interpolate(src_dam_logits, size=src_dam_gt.shape[-2:], mode="bilinear", align_corners=False)

            loc_loss = F.binary_cross_entropy_with_logits(src_loc_logits, src_loc_gt) + dice_loss_from_logits(src_loc_logits, src_loc_gt)
            masked_dam_gt = src_dam_gt.clone()
            masked_dam_gt[src_loc_gt[:, 0] == 0] = IGNORE_INDEX
            dam_loss = F.cross_entropy(src_dam_logits, masked_dam_gt, ignore_index=IGNORE_INDEX)

            src_domain_labels = torch.zeros_like(src_dom_logits)
            tgt_domain_labels = torch.ones_like(tgt_dom_logits)
            dom_loss = domain_criterion(src_dom_logits, src_domain_labels) + domain_criterion(tgt_dom_logits, tgt_domain_labels)

            sup_loss = loc_loss + dam_loss
            loss = sup_loss + dom_loss
        ...
        print(f"[epoch={epoch}] loss={running/max(1,len(src_loader)):.4f} sup={running_sup/max(1,len(src_loader)):.4f} dom={running_dom/max(1,len(src_loader)):.4f} val_f1_avg={val_metrics['f1_avg']:.2f} val_f1_loc={val_metrics['f1_loc']:.2f} val_f1_dam={val_metrics['f1_dam']:.2f}")
