...
class XView2ChangeOSDataset(Dataset):
    def __init__(
        ...
        if not self.pre_masks:
            raise FileNotFoundError(
                f"No pre-disaster target png files found in {self.target_dir}"
            )
        ...
        for pre_m in self.pre_masks:
            ...
            # FIX: keep .png extension when mapping target -> image filename
            pre_img_name = pre_m.name.replace("_target.png", ".png")
            post_img_name = post_m.name.replace("_target.png", ".png")

            pre_img = self.image_dir / pre_img_name
            post_img = self.image_dir / post_img_name

            if not post_m.exists():
                continue
            if not pre_img.exists():
                continue
    ...
    def __getitem__(self, idx):
        ...
        y = {
            "masks": [
                torch.from_numpy((pre_mask > 0).astype(np.uint8)),
                torch.from_numpy(post_mask.astype(np.uint8)),
            ],
            "image_filename": name,
        }
        return x, y
...
def compute_pair_loss(
    ...
    src_post_mask: torch.Tensor,
    tgt_pre: torch.Tensor,
    threshold: float,
    src_samples: int,
    tgt_samples: int,
    stca_loss: str = "lovasz",
) -> torch.Tensor:
    ...
    bsz, channels, _, _ = src_feat.shape
    for b in range(bsz):
        src_valid = src_mask_ds[b] > 1
        ...
        src_vec = src_feat[b].reshape(channels, -1)[:, src_idx]
        tgt_vec = tgt_feat[b].reshape(channels, -1)[:, tgt_idx]
        k = src_vec.shape[1]
        m = tgt_vec.shape[1]
...
def evaluate(model: ChangeOS, loader: DataLoader, device: torch.device) -> Dict[str, float]:
    ...
    f1_dam = (
        0.0
        if min(f1_regular, f1_minor, f1_major, f1_destroyed) == 0
        else (f1_regular * f1_minor * f1_major * f1_destroyed) ** 0.25
    )
...
def main() -> None:
    ...
    print(f"[paths] source={src_dir}")
    print(f"[paths] target={tgt_dir}")
    ...
    for epoch in range(1, args.epochs_source + 1):
        ...
        print(
            ...
            f"val_f1_avg={val_metrics['f1_avg']:.2f}"
        )
        ...
        if val_metrics["f1_avg"] > best_source:
            ...
            torch.save(
                {"model": model.state_dict(), "epoch": epoch, "metrics": val_metrics},
                ckpt_dir / "source_best.pt",
            )
