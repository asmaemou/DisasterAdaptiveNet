...
class ResNet34UNetDANN(nn.Module):
    def __init__(self) -> None:
        super().__init__()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser("ResNet34 + U-Net decoder + DANN | xBD to IDA-BD")
...
@torch.no_grad()
def evaluate_target_test_f1(model: ResNet34UNetDANN, loader: DataLoader, device: torch.device, loc_threshold: float) -> Dict[str, object]:
    model.eval()
...
def main() -> None:
    ...
    model = ResNet34UNetDANN().to(device)
        ...
        for step, src_batch in enumerate(iterator, start=1):
            ...
            if USE_TORCH_AMP:
                with autocast(device_type=device.type, enabled=args.amp and device.type == "cuda"):
                    ...
                    tgt_seg_logits, tgt_domain_logits = model(tgt_img, grl_alpha=grl_alpha)
