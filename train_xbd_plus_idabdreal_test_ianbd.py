...
class BaseDamageDataset(Dataset):
    ...
    def _build_damage_target_from_standard_mask(loc: np.ndarray, dmg: np.ndarray) -> np.ndarray:
        """
        Standardized damage mask convention:
        ...
        Training target convention for damage head:
          0 = no damage
          1 = minor damage
          2 = major damage
          3 = destroyed
...
class XBDOriginalDataset(BaseDamageDataset):
    ...
    def __init__(self, root: str | Path, split: str, image_size: int, training: bool, conditioning_id: int = 0):
        ...
        if not self.images_dir.exists():
            raise FileNotFoundError(f"Expected xBD images dir not found: {self.images_dir}")
        if not self.targets_dir.exists():
            raise FileNotFoundError(f"Expected xBD targets dir not found: {self.targets_dir}")
    ...
    def __getitem__(self, index: int):
        ...
        return self._finalize_item([pre, post], loc, dmg_target, s.stem, "xbd")
...
class StandardSingleImageDamageDataset(BaseDamageDataset):
    """
    Expected layout:
      root/
        train/
    ...
    def __init__(self, root: str | Path, split: str, image_size: int, training: bool, conditioning_id: int = 0, source_name: str = "std"):
        super().__init__(image_size=image_size, training=training, conditioning_id=conditioning_id)
        self.root = Path(root)
        self.split = split
        ...
        for p in (self.images_dir, self.loc_dir, self.dmg_dir):
            if not p.exists():
                raise FileNotFoundError(f"Expected standardized dataset path not found: {p}")

        self.samples = self._collect_samples()
        if not self.samples:
            raise RuntimeError(f"No {self.source_name} samples found under {self.split_root}")

    def _collect_samples(self) -> List[StdSample]:
        ...
        for image_path in image_files:
            ...
            if loc_path.exists() and dmg_path.exists():
                samples.append(StdSample(stem=stem, image_path=image_path, loc_path=loc_path, dmg_path=dmg_path))
        return samples
    ...
    def __getitem__(self, index: int):
        ...
        dmg_target = self._build_damage_target_from_standard_mask(loc, dmg)
        # Duplicate single image to fake a 2-branch pre/post input.
        return self._finalize_item([image, image.copy()], loc, dmg_target, s.stem, self.source_name)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser("Train on xBD + IDA-BD real, test on IAN-BD")
    parser.add_argument("--xbd-root", type=str, default="/homes/j244s673/documents/wsu/phd/xview2")
    parser.add_argument("--ida-real-root", type=str, default="/homes/j244s673/documents/wsu/phd/idabd_real_disasteradaptivenet")
    parser.add_argument("--target-root", type=str, default="/homes/j244s673/documents/wsu/phd/idabd_disasteradaptivenet")
    parser.add_argument("--xbd-train-split", type=str, default="train")
    parser.add_argument("--xbd-val-split", type=str, default="val")
    ...
    parser.add_argument("--output-dir", type=str, default="output/xbd_plus_idabdreal_to_ianbd")
...
def evaluate_target_test_f1(model: nn.Module, loader: DataLoader, device: torch.device, loc_threshold: float) -> Dict[str, object]:
    ...
    for batch in iterator:
        ...
        # convert predicted damage classes 0..3 -> 1..4
        dmg_pred = torch.argmax(dmg_logits, dim=1) + 1

        # only credit damage where buildings are predicted
        dmg_pred = dmg_pred * loc_pred

        # valid GT damage pixels are building pixels with non-ignore labels
...
def write_target_test_outputs(results: Dict[str, object], output_dir: Path) -> None:
    ...
    json_path = scores_dir / "scores_ianbd_test.json"
    txt_path = scores_dir / "scores_ianbd_test.txt"
...
def main() -> None:
    ...
    xbd_train = XBDOriginalDataset(
        args.xbd_root, args.xbd_train_split, args.img_size, True, args.conditioning_id
    )
    xbd_val = XBDOriginalDataset(
    ...
    val_loader = make_eval_loader(ConcatDataset([xbd_val, ida_val]), args.batch_size, args.num_workers)
    target_test_loader = make_eval_loader(target_test, args.batch_size, args.num_workers)

    loc_pos_weight, dmg_class_weights = aggregate_counts([xbd_train, ida_train])

    print(f"xBD train samples: {len(xbd_train)} | xBD val samples: {len(xbd_val)}", flush=True)
    print(f"IDA-BD real train samples: {len(ida_train)} | IDA-BD real val samples: {len(ida_val)}", flush=True)
    print(f"IAN-BD test samples: {len(target_test)}", flush=True)
    ...
    print("Evaluating best checkpoint on IAN-BD test split...", flush=True)
