...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser("Clean rerun: Train DisasterAdaptiveNet on RescueNet-xBD")
    parser.add_argument(
        "--dataset-root",
        type=str,
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="output/rescuenet_xbd_disasteradaptivenet_cleanrerun",
...
def make_dataloaders(args: argparse.Namespace):
    ...
    print(f"Localization pixel counts (pos, neg): {(loc_pos, loc_neg)}", flush=True)
    print(f"Damage class counts [no, minor, major, destroyed]: {train_ds.get_damage_class_counts().tolist()}", flush=True)
    print(f"Damage CE class weights: {dmg_class_weights.tolist()}", flush=True)
...
def main() -> None:
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if score > best_score:
            ...
            print(f"Saved new best checkpoint with val_score={best_score:.4f}", flush=True)
    ...
    print("Evaluating best checkpoint on test split...", flush=True)
    ...
    print(json.dumps({"best_val_score": best_score, "test": test_metrics}, indent=2), flush=True)
