    ...
    def _collect_samples(self) -> List[RescueSample]:
        ...
        for image_path in image_files:
            ...
            if not loc_path.exists() or not dmg_path.exists():
                continue
            samples.append(RescueSample(stem, image_path, loc_path, dmg_path))
        return samples
...
class ConditionalDomainDiscriminator(nn.Module):
    def __init__(self, feature_dim: int = 5, hidden_dim: int = 128) -> None:
        super().__init__()
    ...
    def forward(self, conditioned: torch.Tensor) -> torch.Tensor:
        return self.net(conditioned).squeeze(1)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser("DisasterAdaptiveNet + CDAN for single-source UDA")
...
def build_cdan_condition(logits: torch.Tensor, alpha: float) -> torch.Tensor:
    feat = F.adaptive_avg_pool2d(logits, output_size=1).flatten(1)
    loc_prob = torch.sigmoid(logits[:, 0]).mean(dim=(1, 2), keepdim=True)
    dmg_prob = torch.softmax(logits[:, 1:5], dim=1).mean(dim=(2, 3))
    task_prob = torch.cat([loc_prob, dmg_prob], dim=1)
    conditioned = torch.bmm(task_prob.unsqueeze(2), feat.unsqueeze(1)).flatten(1)
    conditioned = grad_reverse(conditioned, alpha)
    return conditioned
...
def evaluate_target_test_f1(model: nn.Module, loader: DataLoader, device: torch.device, loc_threshold: float) -> Dict[str, object]:
    ...
    damage_f1 = harmonic_mean([no_damage_f1.f1, minor_damage_f1.f1, major_damage_f1.f1, destroyed_f1.f1])
...
def main() -> None:
    ...
    print("Note: this CDAN baseline conditions the domain classifier on pooled DisasterAdaptiveNet output logits and pooled task probabilities.", flush=True)

    source_train = build_labeled_dataset(args, args.source_dataset, get_train_split(args, args.source_dataset), True)
    source_val = build_labeled_dataset(args, args.source_dataset, get_val_split(args, args.source_dataset), False)

    target_unlabeled_splits = [get_train_split(args, args.target_dataset), get_val_split(args, args.target_dataset)]
    ...
    for split in target_unlabeled_splits:
        ...
        target_unlabeled_datasets.append(build_unlabeled_dataset(args, args.target_dataset, split, True))

    target_test = build_labeled_dataset(args, args.target_dataset, get_test_split(args, args.target_dataset), False)

    source_train_loader = make_train_loader(source_train, args.batch_size, args.num_workers)
    source_val_loader = make_eval_loader(source_val, args.batch_size, args.num_workers)
    ...
    loc_pos_weight, dmg_class_weights = aggregate_counts([source_train])

    print(f"Source train samples [{args.source_dataset}]: {len(source_train)}", flush=True)
    print(f"Source val samples   [{args.source_dataset}]: {len(source_val)}", flush=True)
    for split_name, ds in zip(seen_splits, target_unlabeled_datasets):
        print(f"Target unlabeled samples [{args.target_dataset}/{split_name}]: {len(ds)}", flush=True)
    print(f"Target test samples  [{args.target_dataset}]: {len(target_test)}", flush=True)
    ...
    domain_disc = ConditionalDomainDiscriminator(feature_dim=5, hidden_dim=128).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        loc_dice_meter = AverageMeter()
        dmg_ce_meter = AverageMeter()
        dom_src_meter = AverageMeter()
        dom_tgt_meter = AverageMeter()
        ...
        for step, src_batch in enumerate(iterator, start=1):
            ...
            alpha = compute_domain_alpha(epoch, step, steps_per_epoch, args.epochs)
            ...
            if USE_TORCH_AMP:
                with autocast(device_type=device.type, enabled=args.amp and device.type == "cuda"):
                    ...
                    sup_total, loc_bce, loc_dice, dmg_ce = compute_supervised_losses(
                        src_logits, src_loc, src_dmg, loc_criterion, dmg_criterion, device, args
                    )

                    src_cond_feat = build_cdan_condition(src_logits, alpha)
                    tgt_cond_feat = build_cdan_condition(tgt_logits, alpha)

                    src_domain_logits = domain_disc(src_cond_feat)
                    tgt_domain_logits = domain_disc(tgt_cond_feat)
                    src_domain_targets = torch.zeros_like(src_domain_logits)
                    tgt_domain_targets = torch.ones_like(tgt_domain_logits)
            ...
            with torch.no_grad():
                src_domain_pred = (torch.sigmoid(src_domain_logits) > 0.5).float()
                tgt_domain_pred = (torch.sigmoid(tgt_domain_logits) > 0.5).float()
                dom_acc = torch.cat([
                    (src_domain_pred == 0).float(),
                    (tgt_domain_pred == 1).float(),
                ]).mean().item()
            ...
            dom_src_meter.update(domain_src_loss.item(), bs)
            dom_tgt_meter.update(domain_tgt_loss.item(), bs)
            ...
            if use_tqdm:
                iterator.set_postfix(
                    ...
                    doms=f"{dom_src_meter.avg:.4f}",
                    domt=f"{dom_tgt_meter.avg:.4f}",
                ...
                print(
                    ...
                    f"dom_src={dom_src_meter.avg:.4f} | dom_tgt={dom_tgt_meter.avg:.4f} | "
                    f"domacc={dom_acc_meter.avg:.4f} | alpha={alpha:.3f}",
                    flush=True,
                )

        scheduler.step()
        val_metrics = evaluate_source_validation(model, source_val_loader, loc_criterion, dmg_criterion, device, args)
        ...
        row: Dict[str, float | int] = {
            ...
            "train_domain_src": dom_src_meter.avg,
            "train_domain_tgt": dom_tgt_meter.avg,
    ...
    print(f"Evaluating best checkpoint on target test split: {args.target_dataset}", flush=True)
