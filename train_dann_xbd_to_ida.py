    ...
    def _collect_samples(self) -> List[XBDSample]:
        ...
        for post_path in post_images:
            ...
            if not pre_path.exists() or not pre_tgt.exists() or not post_tgt.exists():
                continue
            samples.append(XBDSample(prefix, pre_path, post_path, pre_tgt, post_tgt))
        return samples
...
class DomainDiscriminator(nn.Module):
    def __init__(self, in_dim: int = 5, hidden_dim: int = 64) -> None:
        super().__init__()
    ...
    def forward(self, feat: torch.Tensor, alpha: float) -> torch.Tensor:
        feat = grad_reverse(feat, alpha)
        return self.net(feat).squeeze(1)


def dann_alpha(epoch: int, step: int, num_steps: int, total_epochs: int) -> float:
    p = ((epoch - 1) * num_steps + step) / max(1, total_epochs * num_steps)
    return float(2.0 / (1.0 + math.exp(-10.0 * p)) - 1.0)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser("DANN UDA: xBD source to IDA-BD target")
    ...
    parser.add_argument("--output-dir", type=str, default="output/dann_xbd_to_idabd")
    ...
    parser.add_argument("--domain-hidden-dim", type=int, default=64)
...
def make_train_loader(dataset: Dataset, batch_size: int, num_workers: int, shuffle: bool = True) -> DataLoader:
...
def pooled_domain_features(logits: torch.Tensor) -> torch.Tensor:
...
def evaluate_source_validation(
    ...
    for batch in iterator:
        ...
        loss, loc_bce, loc_dice_loss, dmg_ce = compute_supervised_losses(logits, loc, dmg, loc_criterion, dmg_criterion, device, args)
        logit_loc = logits[:, 0]
        logit_dmg = logits[:, 1:5]
...
def main() -> None:
    ...
    print("Source dataset: xBD", flush=True)
    print("Target dataset: IDA-BD (unlabeled during training)", flush=True)

    source_train = XBDOriginalDataset(args.xbd_root, args.xbd_train_split, args.img_size, True, args.conditioning_id)
    source_val = XBDOriginalDataset(args.xbd_root, args.xbd_val_split, args.img_size, False, args.conditioning_id)
    target_unlabeled = ConcatDataset([
        XBDUnlabeledDataset(args.ida_root, args.ida_train_split, args.img_size, True, args.conditioning_id),
        XBDUnlabeledDataset(args.ida_root, args.ida_val_split, args.img_size, True, args.conditioning_id),
    ])
    target_test = XBDOriginalDataset(args.ida_root, args.ida_test_split, args.img_size, False, args.conditioning_id)

    source_loader = make_train_loader(source_train, args.batch_size, args.num_workers, shuffle=True)
    source_val_loader = make_eval_loader(source_val, args.batch_size, args.num_workers)
    target_loader = make_train_loader(target_unlabeled, args.batch_size, args.num_workers, shuffle=True)
    ...
    print(f"xBD train samples: {len(source_train)} | xBD val samples: {len(source_val)}", flush=True)
    print(f"IDA-BD unlabeled train+val samples: {len(target_unlabeled)}", flush=True)
    print(f"IDA-BD test samples: {len(target_test)}", flush=True)

    model = make_model(device)
    domain_discriminator = DomainDiscriminator(in_dim=5, hidden_dim=args.domain_hidden_dim).to(device)
    optimizer = torch.optim.AdamW(list(model.parameters()) + list(domain_discriminator.parameters()), lr=args.lr, weight_decay=args.weight_decay)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        src_loc_bce_meter = AverageMeter()
        src_loc_dice_meter = AverageMeter()
        src_dmg_ce_meter = AverageMeter()

        iterator = tqdm(source_loader, desc=f"train {epoch}/{args.epochs}") if (tqdm is not None and sys.stderr.isatty()) else source_loader
        ...
        for step, src_batch in enumerate(iterator, start=1):
            ...
            alpha = dann_alpha(epoch, step, num_steps, args.epochs)
            ...
            if USE_TORCH_AMP:
                with autocast(device_type=device.type, enabled=args.amp and device.type == "cuda"):
                    ...
                    sup_total, src_loc_bce, src_loc_dice, src_dmg_ce = compute_supervised_losses(src_logits, src_loc, src_dmg, loc_criterion, dmg_criterion, device, args)
                    src_feat = pooled_domain_features(src_logits)
                    tgt_feat = pooled_domain_features(tgt_logits)
                    src_domain_logits = domain_discriminator(src_feat, alpha)
                    tgt_domain_logits = domain_discriminator(tgt_feat, alpha)
                    src_domain_targets = torch.zeros_like(src_domain_logits)
                    tgt_domain_targets = torch.ones_like(tgt_domain_logits)
                    domain_loss = 0.5 * (
                        domain_criterion(src_domain_logits, src_domain_targets)
                        + domain_criterion(tgt_domain_logits, tgt_domain_targets)
                    )
                    total_loss = sup_total + args.domain_weight * domain_loss
            else:
            ...
            domain_meter.update(domain_loss.item(), bs)
            src_loc_bce_meter.update(src_loc_bce.item(), bs)
            src_loc_dice_meter.update(src_loc_dice.item(), bs)
            src_dmg_ce_meter.update(src_dmg_ce.item(), bs)

            if tqdm is not None and sys.stderr.isatty():
                iterator.set_postfix(loss=f"{total_meter.avg:.4f}", sup=f"{sup_meter.avg:.4f}", dom=f"{domain_meter.avg:.4f}", alpha=f"{alpha:.3f}")
            elif step % 20 == 0 or step == len(source_loader):
                print(
                    f"Epoch {epoch}/{args.epochs} | Step {step}/{len(source_loader)} | "
                    f"loss={total_meter.avg:.4f} | sup={sup_meter.avg:.4f} | dom={domain_meter.avg:.4f} | alpha={alpha:.3f}",
                    flush=True,
                )
        ...
        row: Dict[str, float | int] = {
            ...
            "train_domain_loss": domain_meter.avg,
            "train_source_loc_bce": src_loc_bce_meter.avg,
            "train_source_loc_dice_loss": src_loc_dice_meter.avg,
            "train_source_dmg_ce": src_dmg_ce_meter.avg,
        ...
        history.append(row)
        print(
            f"Epoch {epoch:03d} | train_total={row['train_total_loss']:.4f} | train_dom={row['train_domain_loss']:.4f} | "
        ...
        if improved:
            ...
            best_epoch = epoch
            epochs_without_improvement = 0
            save_checkpoint(output_dir / "checkpoints" / "best.pt", model, domain_discriminator, optimizer, scheduler, scaler, epoch, best_score, best_epoch, args)
        ...
        save_checkpoint(output_dir / "checkpoints" / "last.pt", model, domain_discriminator, optimizer, scheduler, scaler, epoch, best_score, best_epoch, args)
        if epoch % max(1, args.save_every) == 0:
            save_checkpoint(output_dir / "checkpoints" / f"epoch_{epoch:03d}.pt", model, domain_discriminator, optimizer, scheduler, scaler, epoch, best_score, best_epoch, args)
