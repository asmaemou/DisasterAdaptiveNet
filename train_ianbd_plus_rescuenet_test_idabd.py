...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser("Train on IAN-BD + RescueNet-xBD, test on IDA-BD")
    ...
    parser.add_argument("--output-dir", type=str, default="output/ianbd_plus_rescuenet_to_idabd")
...
def main() -> None:
    ...
    train_loader = make_balanced_concat_loader([ian_train, rescuenet_train], args.batch_size, args.num_workers)
    val_loader = make_eval_loader(ConcatDataset([ian_val, rescuenet_val]), args.batch_size, args.num_workers)
    target_test_loader = make_eval_loader(target_test, args.batch_size, args.num_workers)

    loc_pos_weight, dmg_class_weights = aggregate_counts([ian_train, rescuenet_train])
