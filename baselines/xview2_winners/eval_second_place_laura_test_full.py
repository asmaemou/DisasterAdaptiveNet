...
DATASET = Path("/homes/j244s673/documents/wsu/phd/DisasterAdaptiveNet/output/xview2_baseline_datasets/second_place_hurricane_laura_TEST_ONLY")
TEST_IMAGES = DATASET / "images"
TEST_MASKS = DATASET / "masks"

OUT = Path("/homes/j244s673/documents/wsu/phd/DisasterAdaptiveNet/output/xview2_baselines/second_place_hurricane_laura_TEST_ONLY_ZERO_SHOT_full_solution")
...
def to_tensor_numpy(image, conf):
    ...
    # Some original xView2 configs store 6-channel normalization values.
    # Localization models use 3-channel RGB input, so trim mean/std to match image channels.
    # Damage models use 6-channel pre+post input, so keep all 6 values.
...
def predict_localization_one(model, conf, image_pre_rgb, image_post_rgb, model_config):
    # Localization model expects 3 channels: pre-disaster RGB only.
    # The normalization function above trims 6-value mean/std to 3 values.
...
def predict_damage_one(model, conf, image_pre_rgb, image_post_rgb):
    if image_pre_rgb.shape != image_post_rgb.shape:
        image_post_rgb = cv2.resize(
            image_post_rgb,
            (image_pre_rgb.shape[1], image_pre_rgb.shape[0]),
...
def main():
    ...
    print("================================================")
    print("ZERO-SHOT xView2 2nd-place full available ensemble on Hurricane Laura TEST")
    print("No Hurricane Laura TEST fine-tuning is performed.")
    print("================================================")
    ...
    for sample in samples:
        img = read_rgb(sample["pre"])
        h, w = img.shape[:2]
        loc_sum[sample["id"]] = np.zeros((h, w), dtype=np.float32)
        damage_sum[sample["id"]] = np.zeros((5, h, w), dtype=np.float32)
    ...
    with open(OUT / "metrics_summary.txt", "w") as f:
        f.write("2nd-place xView2 full available ensemble ZERO-SHOT on Hurricane Laura TEST\n")
        f.write("No Hurricane Laura TEST fine-tuning used.\n\n")
