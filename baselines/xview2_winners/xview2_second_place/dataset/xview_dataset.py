...
class XviewSingleDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            if "Requested crop size" in str(e) and "larger than the image size" in str(e):
                # Fallback for 512x512 DisasterAdaptiveNet patches.
                # The original xView2 augmentations sometimes request crops larger than our image patches.
                sample = {
                    "image": image_pre,
                    "image1": image_post,
                    "mask": mask,
                ...
                raise
        # Remove variable-length fields before PyTorch DataLoader batching.
        # Each image can have a different number of rectangles, so default_collate fails.
        if isinstance(sample, dict):
            sample.pop("rectangles", None)

        # Force all transformed samples to the same spatial size for PyTorch batching.
        # Some second-place augmentations can output 480x480 while others output 512x512.
        target_h, target_w = 512, 512
        if sample["image"].shape[:2] != (target_h, target_w):
            sample["image"] = cv2.resize(sample["image"], (target_w, target_h), interpolation=cv2.INTER_LINEAR)
            sample["image1"] = cv2.resize(sample["image1"], (target_w, target_h), interpolation=cv2.INTER_LINEAR)
            sample["mask"] = cv2.resize(sample["mask"], (target_w, target_h), interpolation=cv2.INTER_NEAREST)
        ...
        if isinstance(sample, dict) and "rectangles" in sample:
            sample.pop("rectangles", None)
        return sample
