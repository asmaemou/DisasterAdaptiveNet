...
class RandomSizedCropAroundBbox(RandomSizedCrop):
    ...
    def get_params_dependent_on_targets(self, params):
        ...
        if rectangles:
            ...
            start_y = rm.random()
            start_x = rm.random()
        # DisasterAdaptiveNet patches can be exactly the same size as the crop.
        # If image size == crop size, denominator becomes zero, so crop starts at 0.
        crop_width = int(crop_height * self.w2h_ratio)

        h_den = img_height - crop_height
        w_den = img_width - crop_width

        h_start = 0 if h_den <= 0 else (start_y * img_height) / h_den
        w_start = 0 if w_den <= 0 else (start_x * img_width) / w_den
