...
DATASET = Path("/homes/j244s673/documents/wsu/phd/idabd_real_disasteradaptivenet")
TEST_IMAGES = DATASET / "test" / "images"

if (DATASET / "test" / "masks").exists():
    TEST_MASKS = DATASET / "test" / "masks"
else:
    TEST_MASKS = DATASET / "test" / "targets"

FT_EXP = Path("/homes/j244s673/documents/wsu/phd/DisasterAdaptiveNet/output/xview2_baselines/second_place_idabd_FULL_SOLUTION_finetune")
...
def find_finetuned_checkpoint(row):
    ...
    if not folder.exists():
        raise FileNotFoundError(f"Missing fine-tuned folder: {folder}")

    candidates = sorted(folder.glob("*best*")) + sorted(folder.glob("*last*")) + sorted(folder.glob("*"))
    candidates = [p for p in candidates if p.is_file()]

    if not candidates:
        raise FileNotFoundError(f"No checkpoint found inside: {folder}")

    # Prefer best checkpoint if available, otherwise last.
...
def postprocess_prediction(loc_prob, damage_prob):
    damage_1to4 = np.moveaxis(damage_prob[1:5], 0, -1)
...
def main():
    ...
    if args.mode == "zeroshot":
        OUT = Path("/homes/j244s673/documents/wsu/phd/DisasterAdaptiveNet/output/xview2_baselines/second_place_idabd_ZERO_SHOT_full_solution")
        label = "2nd-place xView2 full available ensemble ZERO-SHOT on IDA-BD"
    else:
        OUT = Path("/homes/j244s673/documents/wsu/phd/DisasterAdaptiveNet/output/xview2_baselines/second_place_idabd_FULL_SOLUTION_finetune_test_eval")
        label = "2nd-place xView2 full available ensemble FINE-TUNED on IDA-BD"
    ...
    if args.mode == "finetuned":
        for r in rows:
            r["weight"] = find_finetuned_checkpoint(r)
