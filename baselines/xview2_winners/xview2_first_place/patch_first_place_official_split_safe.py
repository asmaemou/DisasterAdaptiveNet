...
print("Restoring original scripts from git before safe patching...")
...
for name in FILES:
    ...
    # Ensure os module exists for os.environ.
    if "import os\n" not in s:
    ...
    # Train scripts: official val instead of random split.
    s = s.replace(
    ...
    # Tune scripts: official val instead of random split.
    s = s.replace(
    ...
    # Tune scripts: in official mode, train only on train split, not train+val.
    s = s.replace(
...
print("Safe official-split patch complete.")
