...
FILES = TRAIN_TUNE_FILES + PREDICT_FILES

print("Restoring original scripts from git before safe patching...")
subprocess.run(["git", "checkout", "--"] + FILES + ["predict_loc_val.py"], check=True)
...
subprocess.run(["python", "-m", "py_compile"] + FILES + ["predict_loc_val.py"], check=True)
print("Safe official-split patch v2 complete.")
