...
def run_inference_on_dataset(
    ...
    for batch in tqdm(data_loader):
        ...
        for i, image_id in enumerate(image_ids):
            image_uuid = str(image_id)
            image_uuid = os.path.basename(image_uuid)
            image_uuid = image_uuid.replace(".png", "")
            if image_uuid.endswith("_pre_disaster"):
                image_uuid = image_uuid[: -len("_pre_disaster")]
            if image_uuid.endswith("_post_disaster"):
                image_uuid = image_uuid[: -len("_post_disaster")]
            if image_uuid.startswith("test_"):
                image_uuid = image_uuid[len("test_"):]
