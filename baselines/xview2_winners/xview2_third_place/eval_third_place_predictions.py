...
def decode_mask(path: Path, loc=False):
    ...
    # fallback for unusual grayscale colorized masks
    bins = np.linspace(1, 255, 5)
    out[(img > 0) & (img <= bins[1])] = 1
    out[(img > bins[1]) & (img <= bins[2])] = 2
    out[(img > bins[2]) & (img <= bins[3])] = 3
    out[img > bins[3]] = 4
...
for tile_id in ids:
    try:
        ...
        pred_loc_path = find_pred(tile_id, "localization")
        pred_dmg_path = find_pred(tile_id, "damage")

        pred_loc = decode_mask(pred_loc_path, loc=True)
        pred_dmg = decode_mask(pred_dmg_path, loc=False)
...
if missing:
    print("ERROR: missing or failed predictions.")
    for x in missing[:20]:
        print(x)
    raise SystemExit(2)
...
with open(OUT / "metrics_summary.txt", "w") as f:
    f.write(args.title + "\n")
    f.write("No target-dataset fine-tuning used.\n\n")
