#!/usr/bin/env python3
"""
HRTBDA v5 multi-label rare-crop cascaded transformer experiment.
...
This is based on the first HRTBDA-inspired script, with targeted upgrades:
  ...
  - Phase II uses 4 independent sigmoid damage heads, matching the strong-baseline idea
  - Phase II uses rare-damage candidate crop selection so minor/major pixels appear more often
  - Phase II uses inverse-frequency damage weights with optional minor/major boosts
  - Phase II includes an auxiliary localization head to keep building features sharp
  - Gradient accumulation gives a larger effective batch size on one GPU
  - Optional minor/major dilation post-processing can be tested as an ablation
  - v6 adds class-aware paired Damage Object Copy-Paste augmentation for Phase II
...
def _parse_copy_paste_classes(class_text: str) -> Tuple[int, ...]:
    """Parse class names/ids for Damage Object Copy-Paste.

    Returned target5 ids follow xBD foreground encoding:
    ...
    out: List[int] = []
    for raw in class_text.replace(";", ",").replace(" ", ",").split(","):
        key = raw.strip().lower()
        if not key:
            continue
        if key not in mapping:
            raise ValueError(f"Unknown copy-paste class '{raw}'. Use names like minor,major,destroyed.")
        value = mapping[key]
        if value not in out:
            out.append(value)
    return tuple(out) if out else (2, 3, 4)


def _blurred_alpha_from_mask(mask: np.ndarray, ksize: int = 7) -> np.ndarray:
    """Make a soft alpha matte from a boolean mask for less obvious pasted seams."""
    if ksize <= 1:
...
class XBDHRTBDADataset(Dataset):
    ...
    def __init__(
        ...
        damage_copy_paste_prob: float = 0.0,
        damage_copy_paste_classes: str = "minor,major,destroyed",
        damage_copy_paste_max_objects: int = 2,
        damage_copy_paste_min_area: int = 64,
        damage_copy_paste_max_area: int = 25000,
        damage_copy_paste_blend: bool = True,
    ):
        self.root = Path(root)
        ...
        self.damage_copy_paste_prob = float(damage_copy_paste_prob)
        self.damage_copy_paste_class_ids = _parse_copy_paste_classes(damage_copy_paste_classes)
        self.damage_copy_paste_max_objects = int(damage_copy_paste_max_objects)
        self.damage_copy_paste_min_area = int(damage_copy_paste_min_area)
        self.damage_copy_paste_max_area = int(damage_copy_paste_max_area)
        self.damage_copy_paste_blend = bool(damage_copy_paste_blend)
    ...
    def _load_resized_pair_for_copy_paste(self, index: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        """Load another sample at full training size for copy-paste donors.

        This intentionally skips random augmentation so the donor object remains coherent.
        The receiver already gets geometric/photometric augmentation, and pasted objects are
        randomly translated, optionally blended, then rare-cropped.
        """
        ...
        return pre, post, loc_raw, target5

    def _sample_damage_component(
        self,
        target5: np.ndarray,
    ) -> Optional[Tuple[int, np.ndarray, Tuple[int, int, int, int]]]:
        """Sample one connected component from allowed damage classes.

        Returns: (class_id, component_mask_full_image, bbox_y0_x0_h_w)
        """
        class_ids = list(self.damage_copy_paste_class_ids)
        random.shuffle(class_ids)

        # Bias toward rare/difficult classes first when they are allowed.
        class_priority = [4, 2, 3, 1]
        class_ids = sorted(class_ids, key=lambda c: class_priority.index(c) if c in class_priority else 99)

        components: List[Tuple[int, np.ndarray, Tuple[int, int, int, int], int]] = []
        h, w = target5.shape[:2]
        max_area = self.damage_copy_paste_max_area

        for cls_id in class_ids:
            binary = (target5 == cls_id).astype(np.uint8)
            if binary.sum() < self.damage_copy_paste_min_area:
                continue
            n_labels, labels, stats, _ = cv2.connectedComponentsWithStats(binary, connectivity=8)
            for label_id in range(1, n_labels):
                x, y, bw, bh, area = stats[label_id]
                area = int(area)
                if area < self.damage_copy_paste_min_area:
                    continue
                if max_area > 0 and area > max_area:
                    continue
                # Avoid huge connected regions that are usually mask artifacts.
                if bw >= 0.75 * w or bh >= 0.75 * h:
                    continue
                comp_mask = labels == label_id
                components.append((cls_id, comp_mask, (int(y), int(x), int(bh), int(bw)), area))

        if not components:
            return None

        # Sample proportional to sqrt(area) so tiny objects are possible, but larger valid
        # objects are not ignored.
        weights = np.array([math.sqrt(max(1, item[3])) for item in components], dtype=np.float64)
        weights = weights / weights.sum()
        chosen_idx = int(np.random.choice(len(components), p=weights))
        cls_id, comp_mask, bbox, _area = components[chosen_idx]
        return cls_id, comp_mask, bbox

    def _apply_damage_object_copy_paste(
        self,
        pre: np.ndarray,
        post: np.ndarray,
        loc_raw: np.ndarray,
        ...
        """Class-aware paired Copy-Paste for rare building damage.

        It copies the same building/damage object from donor pre/post images and donor mask
        into background of the receiver. This keeps pre/post damage evidence paired while
        increasing exposure to minor/major/destroyed buildings.
        """
        if self.damage_copy_paste_prob <= 0.0 or len(self.samples) < 2:
            return pre, post, loc_raw, target5
        if np.random.rand() >= self.damage_copy_paste_prob:
            return pre, post, loc_raw, target5

        h, w = target5.shape[:2]
        out_pre = pre.copy()
        out_post = post.copy()
        out_loc = loc_raw.copy()
        out_target = target5.copy()

        max_objects = max(1, self.damage_copy_paste_max_objects)
        pasted = 0

        for _obj in range(max_objects):
            donor_idx = np.random.randint(0, len(self.samples))
            if donor_idx == current_index and len(self.samples) > 1:
                donor_idx = (donor_idx + np.random.randint(1, len(self.samples))) % len(self.samples)

            d_pre, d_post, d_loc, d_target = self._load_resized_pair_for_copy_paste(donor_idx)
            sampled = self._sample_damage_component(d_target)
            if sampled is None:
                continue

            cls_id, comp_mask_full, (sy, sx, bh, bw) = sampled
            if bh <= 1 or bw <= 1 or bh > h or bw > w:
                continue

            comp_patch = comp_mask_full[sy:sy + bh, sx:sx + bw]
            src_pre_patch = d_pre[sy:sy + bh, sx:sx + bw]
            src_post_patch = d_post[sy:sy + bh, sx:sx + bw]
            src_loc_patch = d_loc[sy:sy + bh, sx:sx + bw]
            src_tgt_patch = d_target[sy:sy + bh, sx:sx + bw]

            # Try to paste into background to avoid corrupting existing labels.
            dest_y = dest_x = None
            dest_mask = None
            src_area = int(comp_patch.sum())
            min_keep = max(self.damage_copy_paste_min_area, int(0.70 * src_area))
            for _try in range(60):
                y = np.random.randint(0, h - bh + 1)
                x = np.random.randint(0, w - bw + 1)
                receiver_bg = out_target[y:y + bh, x:x + bw] == 0
                keep = comp_patch & receiver_bg
            ...
            if dest_mask is None:
                # Fall back to paste in same relative area if it stays mostly background.
                y = min(max(0, sy), h - bh)
                x = min(max(0, sx), w - bw)
                receiver_bg = out_target[y:y + bh, x:x + bw] == 0
                keep = comp_patch & receiver_bg
                if int(keep.sum()) < self.damage_copy_paste_min_area:
                    continue
                dest_y, dest_x = y, x
                dest_mask = keep

            y, x = int(dest_y), int(dest_x)
            dp = out_pre[y:y + bh, x:x + bw]
            dpo = out_post[y:y + bh, x:x + bw]
            dl = out_loc[y:y + bh, x:x + bw]
            dt = out_target[y:y + bh, x:x + bw]

            if self.damage_copy_paste_blend:
                alpha = _blurred_alpha_from_mask(dest_mask, ksize=7)
                # Keep labels hard but make RGB seams softer.
                dp[:] = np.clip(alpha * src_pre_patch.astype(np.float32) + (1.0 - alpha) * dp.astype(np.float32), 0, 255).astype(np.uint8)
                dpo[:] = np.clip(alpha * src_post_patch.astype(np.float32) + (1.0 - alpha) * dpo.astype(np.float32), 0, 255).astype(np.uint8)
            else:
                dp[dest_mask] = src_pre_patch[dest_mask]
                dpo[dest_mask] = src_post_patch[dest_mask]

            # Hard update labels exactly where the pasted donor object exists.
            # Target class is from donor mask to preserve no/minor/major/destroyed label.
            valid_src = dest_mask & np.isin(src_tgt_patch, [1, 2, 3, 4])
            dl[valid_src] = np.maximum(dl[valid_src], src_loc_patch[valid_src])
            dl[valid_src] = np.maximum(dl[valid_src], 1)
            dt[valid_src] = src_tgt_patch[valid_src]

            pasted += 1

        return out_pre, out_post, out_loc, out_target
    ...
    def __getitem__(self, index: int) -> Dict[str, torch.Tensor | str]:
        ...
        if self.training and self.damage_copy_paste_prob > 0.0:
            pre, post, loc_raw, target5 = self._apply_damage_object_copy_paste(
                pre=pre,
                post=post,
                loc_raw=loc_raw,
                target5=target5,
...
def make_loaders(args: argparse.Namespace, phase2_training: bool = False) -> Tuple[DataLoader, DataLoader, DataLoader, XBDHRTBDADataset]:
    ...
    train_ds = XBDHRTBDADataset(
        ...
        damage_copy_paste_prob=args.damage_copy_paste_prob if phase2_training else 0.0,
        damage_copy_paste_classes=args.damage_copy_paste_classes,
        damage_copy_paste_max_objects=args.damage_copy_paste_max_objects,
        damage_copy_paste_min_area=args.damage_copy_paste_min_area,
        damage_copy_paste_max_area=args.damage_copy_paste_max_area,
        damage_copy_paste_blend=not args.disable_damage_copy_paste_blend,
    )
...
def train_phase2(args: argparse.Namespace, device: torch.device, phase1_ckpt: Optional[Path]) -> Path:
    ...
    print(f"Damage Copy-Paste probability: {args.damage_copy_paste_prob}", flush=True)
    print(f"Damage Copy-Paste classes: {args.damage_copy_paste_classes}", flush=True)
    print(
...
def test_phase2(args: argparse.Namespace, device: torch.device, checkpoint_path: Path, phase1_ckpt: Path) -> None:
    ...
    json_path = scores_dir / "scores_xbd_test_hrtbda_v6_damage_copypaste.json"
    txt_path = scores_dir / "scores_xbd_test_hrtbda_v6_damage_copypaste.txt"
    summary_path = scores_dir / "summary_hrtbda_v6_damage_copypaste.txt"
    ...
    lines = [
        f"Experiment: HRTBDA v6 damage-copy-paste multi-label rare-crop cascade {train_splits_text} -> {args.val_split} -> {args.test_split}",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--damage-copy-paste-prob", type=float, default=0.0,
                        help="Probability of applying paired Damage Object Copy-Paste during Phase II training.")
    parser.add_argument("--damage-copy-paste-classes", type=str, default="minor,major,destroyed",
                        help="Comma-separated target classes to copy-paste, e.g. minor,major,destroyed.")
    parser.add_argument("--damage-copy-paste-max-objects", type=int, default=2,
                        help="Maximum pasted damage objects per Phase II training sample.")
    parser.add_argument("--damage-copy-paste-min-area", type=int, default=64,
                        help="Minimum connected-component area for copy-paste donors.")
    parser.add_argument("--damage-copy-paste-max-area", type=int, default=25000,
                        help="Maximum connected-component area for copy-paste donors; <=0 disables max cap.")
    parser.add_argument("--disable-damage-copy-paste-blend", action="store_true",
                        help="Disable soft RGB blending for Damage Object Copy-Paste.")
...
def main() -> None:
    ...
    print("===== HRTBDA V6 DAMAGE COPY-PASTE MULTI-LABEL RARE-CROP CASCADED TRAINING =====", flush=True)
    ...
    print(f"Damage Copy-Paste prob: {args.damage_copy_paste_prob}", flush=True)
    print(f"Damage Copy-Paste classes: {args.damage_copy_paste_classes}", flush=True)
    print(f"Damage Copy-Paste max objects: {args.damage_copy_paste_max_objects}", flush=True)
    ...
    print("Architecture: HRTBDA v6 4-branch HRNet-style + DCSwin + CSF fusion + Damage Object Copy-Paste", flush=True)
