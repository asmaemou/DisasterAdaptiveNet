...
class OffsetUnifiedFocalLoss(nn.Module):
    ...
    def forward(self, logits: torch.Tensor, target: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # Extra guard: if AMP/model dynamics create inf/nan logits, do not let
        # them enter sigmoid/softplus where they can poison the loss.
        logits_f = torch.nan_to_num(logits.float(), nan=0.0, posinf=20.0, neginf=-20.0)
        logits_f = logits_f.clamp(min=-20.0, max=20.0)
        target_f = target.float()
        prob = torch.sigmoid(logits_f).clamp(min=self.eps, max=1.0 - self.eps)
        ...
        # Final guard against rare numerical issues. This should almost never
        # activate, but it keeps one pathological batch from killing a 70-hour job.
        cfl = torch.nan_to_num(cfl, nan=0.0, posinf=1e4, neginf=0.0)
        cftl = torch.nan_to_num(cftl, nan=0.0, posinf=1e4, neginf=0.0)
...
def backward_step(
    ...
    """Backward + optimizer step with non-finite gradient protection."""
    ...
    if args.max_grad_norm is not None and args.max_grad_norm > 0:
        ...
        if not torch.isfinite(total_norm):
            print(
                f"WARNING: non-finite gradient norm detected ({total_norm}); skipping optimizer step.",
                flush=True,
            )
            ...
            if USE_TORCH_AMP:
                with autocast(device_type=device.type, enabled=args.amp and device.type == "cuda"):
                    ...
                    if not torch.isfinite(loss):
                        print(f"WARNING: Non-finite Phase I loss at epoch {epoch}, step {step}: loss={loss.item()}, focal={focal.item()}, dice={dice.item()}; skipping batch.", flush=True)
                        optimizer.zero_grad(set_to_none=True)
                        continue
            else:
        ...
        for step, batch in enumerate(iterator, start=1):
            ...
            if USE_TORCH_AMP:
                with autocast(device_type=device.type, enabled=args.amp and device.type == "cuda"):
                    ...
                    if not torch.isfinite(loss):
                        print(f"WARNING: Non-finite Phase II loss at epoch {epoch}, step {step}: loss={loss.item()}, component1={focal.item()}, component2={dice.item()}; skipping batch.", flush=True)
                        optimizer.zero_grad(set_to_none=True)
                        continue
            else:
                with autocast(enabled=args.amp and device.type == "cuda"):
