...
What this version uses:
  ...
  - Resume support: --resume-phase1-from can continue Phase I from phase1_last.pt.
"""
...
def train_phase1(args: argparse.Namespace, device: torch.device) -> Path:
    ...
    # Load previous Phase I history if present. This keeps the final history file continuous
    # after resuming from a SLURM time-limit interruption.
    ...
    if resume_path is not None:
        ...
        # Keep only history rows that are truly before the next epoch we are going to run.
        # This avoids duplicate rows if a previous job wrote history but died before saving
        # a matching last checkpoint.
