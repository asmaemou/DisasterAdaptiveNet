#!/usr/bin/env python3
"""
HRTBDA v5 full two-phase transformer experiment from scratch.
...
This is based on the first HRTBDA-inspired script, with targeted upgrades:
  ...
  - Phase I is trained from scratch and best checkpoint initializes Phase II Siamese damage model
...
class ChangeAwareMSDFFusionModule(nn.Module):
    """
    HRTBDA v7: Change-Aware Multi-Scale Difference Fusion (MSDF).

    At each backbone scale, this module keeps the original CSF path and adds
    an explicit learned pre/post change path using:
      - f_post - f_pre      (signed difference)
      - |f_post - f_pre|    (absolute difference)
      - [f_pre, f_post]     (original semantic features)

    The learned difference branch gives the decoder a stronger cue for subtle
    damage changes, especially minor/major damage.
    """
    def __init__(self, channels: int):
        super().__init__()
        self.csf = CSFModule(channels)
    ...
    def forward(self, fpre: torch.Tensor, fpost: torch.Tensor) -> torch.Tensor:
        signed_diff = fpost - fpre
        abs_diff = torch.abs(signed_diff)

        diff_feat = self.diff_encoder(torch.cat([fpre, fpost, abs_diff, signed_diff], dim=1))
        diff_feat = diff_feat * self.change_gate(diff_feat)

        csf_feat = self.csf(fpre, fpost)
        return self.out_fuse(torch.cat([csf_feat, diff_feat], dim=1))


class HRTBDAPhase2(nn.Module):
    """Phase II damage model for HRTBDA v7-MSDF.

    Outputs:
      damage_logits: [B, 4, H, W] independent sigmoid damage channels
      aux_loc:       [B, H, W] auxiliary building localization logit

    Final localization still comes from the Phase-I localization model during
    validation/test. This class only changes Phase II feature fusion.
    """
    def __init__(self, base_channels: int, decoder_channels: int, window_size: int, num_classes: int = 4):
        ...
        self.change_fusion = nn.ModuleList([
            ChangeAwareMSDFFusionModule(c) for c in self.backbone.channels
        ])
    ...
    def forward(self, pre: torch.Tensor, post: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        fused = [module(a, b) for module, a, b in zip(self.change_fusion, fpre, fpost)]
...
def test_phase2(args: argparse.Namespace, device: torch.device, checkpoint_path: Path, phase1_ckpt: Path) -> None:
    ...
    json_path = scores_dir / "scores_xbd_test_hrtbda_v7_msdf_full_two_stage.json"
    txt_path = scores_dir / "scores_xbd_test_hrtbda_v7_msdf_full_two_stage.txt"
    summary_path = scores_dir / "summary_hrtbda_v7_msdf_full_two_stage.txt"
    ...
    lines = [
        f"Experiment: HRTBDA v7-MSDF multi-label rare-crop cascade {train_splits_text} -> {args.val_split} -> {args.test_split}",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--batch-size", type=int, default=2, help="Backward-compatible alias; prefer --phase1-batch-size and --phase2-batch-size.")
    parser.add_argument("--phase1-batch-size", type=int, default=1)
...
def main() -> None:
    ...
    print("===== HRTBDA V7-MSDF FULL TWO-STAGE TRAINING =====", flush=True)
    ...
    print("Architecture: HRTBDA v7 4-branch HRNet-style + DCSwin + CSF + Change-Aware MSDF", flush=True)
