#!/usr/bin/env python3
"""Train the ResNet34-Swin-Tiny cross-attention model on Pakistan Flooding."""
...
class PakistanFloodingDataset(BASE_DATASET):
    """Standard paired loader for Pakistan Flooding train/val/test data."""

    def __getitem__(self, index):
        item = super().__getitem__(index)
        # The common runner requires this field, but the selected architecture
        # is hazard-agnostic and explicitly ignores the condition value.
        item["cond_id"] = torch.tensor([0], dtype=torch.long)
        return item


def write_pakistan_flooding_outputs(
    results, output_dir: Path, best_epoch: int, best_threshold: float
):
    """Write compatibility files and unambiguous Pakistan Flooding results."""
    BASE_WRITE_OUTPUTS(results, output_dir, best_epoch, best_threshold)
    scores_dir = output_dir / "scores"
    json_path = scores_dir / "scores_pakistan_flooding_test.json"
    text_path = scores_dir / "scores_pakistan_flooding_test.txt"
    summary_path = scores_dir / "summary_pakistan_flooding.txt"
    ...
    lines = [
        ...
        "Protocol: Pakistan Flooding train -> validation selection -> untouched test",
        "xBD data or xBD-trained checkpoints used: No",
        f"Best epoch selected on Pakistan Flooding validation: {best_epoch}",
        f"Best localization threshold selected on Pakistan Flooding validation: {best_threshold:.4f}",
    ...
    print(f"Wrote Pakistan Flooding JSON:    {json_path}", flush=True)
    print(f"Wrote Pakistan Flooding summary: {summary_path}", flush=True)
...
if __name__ == "__main__":
    ...
    runner.XBDOriginalDataset = PakistanFloodingDataset
    ...
    runner.write_final_outputs = write_pakistan_flooding_outputs
    runner.main()
