#!/usr/bin/env python3
"""
HRTBDA v2 — key improvements over v1:
 ...
 11. Phase II now uses a foreground-only damage head: one localization head
     predicts building/background and one 4-class damage head predicts only
     no/minor/major/destroyed on building pixels. This prevents the huge
     background class from dominating damage classification.

 12. Rare-damage weighted sampling oversamples tiles containing minor/major
     damage so Phase II sees the hard classes more often.

 13. ChangeFormer-style learnable pre/post difference fusion concatenates
     pre features, post features, absolute difference, and feature interaction
     instead of relying only on raw absolute difference.

 14. Test-time augmentation (TTA) can average logits over flips or D4
     transforms during final evaluation.
"""
...
class XBDHRTBDADataset(Dataset):
    ...
    def sample_damage_counts(self, index: int) -> np.ndarray:
        """
        Count foreground damage pixels in one sample.

        Returns counts for target5 classes:
    ...
    def make_rare_damage_sample_weights(
        ...
        """
        Per-tile sampling weights for Phase II.

        This oversamples tiles that contain rare/hard foreground classes.
        The weights are tile-level weights, not pixel-level loss weights.
        """
        weights = np.ones(len(self.samples), dtype=np.float64)
        for i in range(len(self.samples)):
            c = self.sample_damage_counts(i)
            if c[2] > 0:
                weights[i] = max(weights[i], float(minor_weight))
            if c[3] > 0:
                weights[i] = max(weights[i], float(major_weight))
            if c[4] > 0:
                weights[i] = max(weights[i], float(destroyed_weight))
        return weights
...
class EnhancedDifferenceFusion(nn.Module):
    """
    ChangeFormer-style learnable pre/post feature fusion.

    Instead of using only abs(post - pre), this module lets the network learn
    the best distance metric from:
      pre attention features,
      post attention features,
      absolute feature difference,
      feature interaction.

    This is aimed at minor/major damage, where the useful signal can be weak
    and not always captured by raw absolute difference alone.
    """
    ...
    def _attention(self, x: torch.Tensor) -> torch.Tensor:
        ...
        sp = self.spatial_conv(torch.cat([
    ...
    def forward(self, fpre: torch.Tensor, fpost: torch.Tensor) -> torch.Tensor:
        ...
        inter = apre * apost
        return self.fuse(torch.cat([apre, apost, diff, inter], dim=1))


# Backward-compatible name in case old checkpoints/logs mention CSF.
CSFModule = EnhancedDifferenceFusion
...
class HRTBDAPhase2(nn.Module):
    """
    Phase II uses two heads:

      1. loc_decoder: binary building/background localization head.
         It is initialized from the Phase I decoder when possible.

      2. damage_decoder: foreground-only 4-class damage head.
         Classes are [no damage, minor, major, destroyed] and the loss ignores
         background pixels. This prevents the background class from dominating
         damage learning.
    """
    def __init__(self, base_channels, decoder_channels, window_size, num_damage_classes=4):
        super().__init__()
        self.backbone       = HighResolutionTransformerBackbone(3, base_channels, window_size)
        self.diff_fusion    = nn.ModuleList([EnhancedDifferenceFusion(c) for c in self.backbone.channels])
        self.damage_decoder = MultiScaleDecoder(self.backbone.channels, decoder_channels, num_damage_classes)
        self.loc_decoder    = MultiScaleDecoder(self.backbone.channels, decoder_channels, 1)

    def forward(self, pre, post):
        fpre  = self.backbone(pre)
        fpost = self.backbone(post)

        # Foreground localization is predicted from pre-disaster features because
        # building contours are cleaner before the disaster.
        logits_loc = self.loc_decoder(fpre, output_size=pre.shape[-2:]).squeeze(1)

        # Damage classification is predicted from learnable pre/post difference fusion.
        fused = [m(a, b) for m, a, b in zip(self.diff_fusion, fpre, fpost)]
        logits_damage = self.damage_decoder(fused, output_size=pre.shape[-2:])
        return logits_damage, logits_loc
...
# FIX 3 — foreground-only inverse-frequency weights + rare-class boosts
def make_damage_class_weights(
    dataset: XBDHRTBDADataset,
    minor_boost: float = 3.0,
    major_boost: float = 1.5,
    destroyed_boost: float = 1.0,
) -> torch.Tensor:
    """
    Build weights for the 4 foreground damage classes only:
      0 no-damage, 1 minor, 2 major, 3 destroyed.

    Background is intentionally excluded because Phase II now has a separate
    localization head. This prevents the damage head from learning an easy
    background-vs-building shortcut.
    """
    counts5 = dataset.class5_counts().astype(np.float64)
    counts  = counts5[1:5]  # no, minor, major, destroyed
    ...
    print(
        "  foreground damage class weights [no,minor,major,dest]: "
        f"{weights.tolist()} | counts={counts.astype(int).tolist()}",
        flush=True,
    )
...
def target5_to_damage4(target5: torch.Tensor, ignore_index: int = 255) -> torch.Tensor:
    """
    Convert target5 labels to foreground-only damage labels.

      target5: 0 background, 1 no, 2 minor, 3 major, 4 destroyed, 255 ignore
      output: 0 no, 1 minor, 2 major, 3 destroyed, 255 ignore/background
    """
    out = torch.full_like(target5, int(ignore_index))
...
def _tta_transforms(mode: str) -> List[Tuple[int, bool]]:
    """
    Return (rot_k, hflip) transforms.

    mode:
      none: no TTA
      flip: original + H/V/HV flips
      d4: rotations 0/90/180/270 plus horizontal flips
    """
    mode = str(mode or "none").lower()
    if mode == "none":
        return [(0, False)]
    if mode == "flip":
        # Represent V and HV via rotations/flips would be awkward, so they are
        # handled explicitly in predict_phase2_logits_tta.
        return [(0, False)]
    if mode == "d4":
        return [(k, False) for k in range(4)] + [(k, True) for k in range(4)]
    raise ValueError(f"Unknown TTA mode: {mode}")


def _apply_rot_flip(x: torch.Tensor, rot_k: int = 0, hflip: bool = False) -> torch.Tensor:
...
def _invert_rot_flip(x: torch.Tensor, rot_k: int = 0, hflip: bool = False) -> torch.Tensor:
    y = torch.flip(x, dims=(-1,)) if hflip else x
    if rot_k:
        y = torch.rot90(y, k=-rot_k, dims=(-2, -1))
    return y


@torch.no_grad()
def predict_phase2_logits(model, pre, post, tta_mode: str = "none"):
    """
    Return averaged (damage_logits, loc_logits).

    damage_logits: [B, 4, H, W]
    loc_logits:    [B, H, W]
    """
    ...
    if mode == "flip":
        transforms = [
            ...
            ("h",    lambda x: torch.flip(x, dims=(-1,)), lambda x: torch.flip(x, dims=(-1,))),
            ("v",    lambda x: torch.flip(x, dims=(-2,)), lambda x: torch.flip(x, dims=(-2,))),
            ("hv",   lambda x: torch.flip(x, dims=(-2,-1)), lambda x: torch.flip(x, dims=(-2,-1))),
        ]
        dmg_acc = loc_acc = None
        for _, fwd, inv in transforms:
            d, l = model(fwd(pre), fwd(post))
            d = inv(d)
            l = inv(l.unsqueeze(1)).squeeze(1)
            dmg_acc = d if dmg_acc is None else dmg_acc + d
            loc_acc = l if loc_acc is None else loc_acc + l
        return dmg_acc / len(transforms), loc_acc / len(transforms)

    # D4: rotations and horizontal flips.
    transforms = _tta_transforms(mode)
    dmg_acc = loc_acc = None
    for rot_k, hflip in transforms:
        tpre  = _apply_rot_flip(pre,  rot_k=rot_k, hflip=hflip)
        tpost = _apply_rot_flip(post, rot_k=rot_k, hflip=hflip)
        d, l = model(tpre, tpost)
        d = _invert_rot_flip(d, rot_k=rot_k, hflip=hflip)
        l = _invert_rot_flip(l.unsqueeze(1), rot_k=rot_k, hflip=hflip).squeeze(1)
        dmg_acc = d if dmg_acc is None else dmg_acc + d
        loc_acc = l if loc_acc is None else loc_acc + l
    return dmg_acc / len(transforms), loc_acc / len(transforms)
...
def evaluate_phase2(
    ...
    for batch in loader:
        ...
        logits_damage, logits_loc = predict_phase2_logits(model, pre, post, tta_mode=tta_mode)

        loc_pred = (torch.sigmoid(logits_loc) > float(loc_threshold)).long()
        dmg_pred = torch.argmax(logits_damage, dim=1) + 1  # back to target5 classes 1..4

        # Full prediction map for xView2-style accounting.
        pred = torch.where(loc_pred > 0, dmg_pred, torch.zeros_like(dmg_pred))
    ...
    nd, mi, ma, de = [
        F1Recorder(cls_counts[c]["tp"], cls_counts[c]["fp"], cls_counts[c]["fn"])
        for c in [1,2,3,4]
    ]
    ...
    return {
        ...
        "damage_f1": dmg_f1,
        "damage_f1_no_damage": nd.f1,
        "damage_f1_minor_damage": mi.f1,
        "damage_f1_major_damage": ma.f1,
        "damage_f1_destroyed": de.f1,
        "details": {
            "localization": loc.as_dict(),
            "no_damage": nd.as_dict(),
            "minor_damage": mi.as_dict(),
            "major_damage": ma.as_dict(),
...
def load_phase1_backbone_into_phase2(phase2, phase1_ckpt, device):
    ...
    # Load Phase I backbone into Phase II backbone.
    backbone_state = {
        k.replace("backbone.", "", 1): v
        for k, v in ckpt["model"].items()
        if k.startswith("backbone.")
    }
    miss, unex = phase2.backbone.load_state_dict(backbone_state, strict=False)
    print(
        f"Loaded Phase I backbone → Phase II | missing={len(miss)} unexpected={len(unex)}",
        flush=True,
    )

    # Load Phase I localization decoder into Phase II localization head.
    # This is possible because both decoders output one channel.
    decoder_state = {
        k.replace("decoder.", "", 1): v
        for k, v in ckpt["model"].items()
        if k.startswith("decoder.")
    }
    if decoder_state and hasattr(phase2, "loc_decoder"):
        miss_d, unex_d = phase2.loc_decoder.load_state_dict(decoder_state, strict=False)
        print(
            f"Loaded Phase I decoder → Phase II loc head | "
...
def make_loaders(args, use_rare_sampler: bool = False):
    ...
    if use_rare_sampler:
        weights = train_ds.make_rare_damage_sample_weights(
            minor_weight=args.minor_sample_weight,
            major_weight=args.major_sample_weight,
            destroyed_weight=args.destroyed_sample_weight,
        )
        ...
        train_loader = DataLoader(train_ds, sampler=sampler, shuffle=False, drop_last=True, **kw)
        unique, counts = np.unique(weights, return_counts=True)
        summary = ", ".join([f"{float(u):.2f}x:{int(c)}" for u, c in zip(unique, counts)])
        print(f"Rare-damage sampler enabled | tile weights: {summary}", flush=True)
    else:
...
def train_phase1(args, device) -> Path:
    ...
    # Only load old history when explicitly resuming. A fresh experiment should
    # not inherit stale best metrics or stale thresholds from an earlier run.
    if resume_path and hist_path.exists():
        try:
...
def train_phase2(args, device, phase1_ckpt: Optional[Path]) -> Path:
    ...
    train_loader, val_loader, _, train_ds = make_loaders(args, use_rare_sampler=args.rare_sampler)
    print(f"Train={len(train_loader.dataset)}  Val={len(val_loader.dataset)}", flush=True)
    ...
    for epoch in range(1, args.phase2_epochs + 1):
        ...
        for step, batch in enumerate(it, 1):
            ...
            ctx = (autocast(device_type=device.type, enabled=args.amp and device.type=="cuda")
                   if USE_TORCH_AMP else autocast(enabled=args.amp and device.type=="cuda"))
            with ctx:
                logits_damage, logits_loc = model(pre, post)

                # Foreground-only damage labels: background/ignore -> 255.
                target_damage = target5_to_damage4(target, ignore_index=255)

                loss_cls, focal, dice = criterion(logits_damage, target_damage)

                # Phase II localization head keeps building extraction sharp.
                loss_loc, _, _ = aux_crit(logits_loc, loc)
                loss = args.cls_loss_weight * loss_cls + args.aux_loc_weight * loss_loc
        ...
        scheduler.step()
        vres  = evaluate_phase2(model, val_loader, device, loc_threshold=args.phase2_loc_threshold, tta_mode=args.val_tta)
        vs    = float(vres["score"])
...
def test_phase2(args, device, checkpoint_path: Path) -> None:
    ...
    results   = evaluate_phase2(model, test_loader, device, loc_threshold=args.phase2_loc_threshold, tta_mode=args.test_tta)
    ...
    lines = [
        ...
        f"Test TTA mode: {args.test_tta}",
        f"Phase II loc threshold: {args.phase2_loc_threshold}",
...
def parse_args():
    ...
    p.add_argument("--minor-damage-boost", type=float, default=3.0)   # foreground damage loss boost
    p.add_argument("--major-damage-boost", type=float, default=1.5)   # foreground damage loss boost
    p.add_argument("--destroyed-damage-boost", type=float, default=1.0)
    p.add_argument("--cutmix-prob",     type=float, default=0.3)      # CutMix probability

    # Rare-damage tile sampler for Phase II.
    p.add_argument("--rare-sampler", action="store_true")
    p.add_argument("--minor-sample-weight", type=float, default=4.0)
    p.add_argument("--major-sample-weight", type=float, default=2.5)
    p.add_argument("--destroyed-sample-weight", type=float, default=1.5)

    # Foreground-only Phase II inference.
    p.add_argument("--phase2-loc-threshold", type=float, default=0.5)

    # TTA mode: none, flip, or d4. Use none for validation unless you can afford it.
    p.add_argument("--val-tta",  type=str, default="none", choices=["none", "flip", "d4"])
    p.add_argument("--test-tta", type=str, default="none", choices=["none", "flip", "d4"])
...
def main():
    ...
    print("Backbone: 4-stage HRNet + DCSwin + learnable diff fusion | "
          "Fixes: focal-pt, foreground-only damage head, rare sampler, "
          "warmup-cosine LR, CutMix, TTA, scale-jitter aug", flush=True)
    print("=====================", flush=True)
