...
def test_phase2(args: argparse.Namespace, device: torch.device, checkpoint_path: Path) -> None:
    ...
    json_path = scores_dir / "scores_xbd_test_hrtbda_v2_4branch_warmup.json"
    txt_path = scores_dir / "scores_xbd_test_hrtbda_v2_4branch_warmup.txt"
    summary_path = scores_dir / "summary_hrtbda_v2_4branch_warmup.txt"
    ...
    lines = [
        f"Experiment: HRTBDA v2 4-branch warmup xBD {train_splits_text} -> {args.val_split} -> {args.test_split}",
...
def main() -> None:
    ...
    print("===== HRTBDA V2 4-BRANCH WARMUP AMP XBD TRAINING =====", flush=True)
