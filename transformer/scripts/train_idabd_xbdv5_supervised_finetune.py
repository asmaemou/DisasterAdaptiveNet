#!/usr/bin/env python3
"""
HRTBDA v5 supervised transfer learning from xBD to IDA-BD.

Pipeline:
  1) Load the deterministic IDA-BD 80/10/10 split.
  2) Initialize Phase I from the xBD-trained Phase-I checkpoint and fine-tune on IDA-BD train.
  3) Initialize Phase II from the xBD-trained v5 Phase-II checkpoint and fine-tune on IDA-BD train.
  4) Select checkpoints/settings on IDA-BD validation.
  5) Test once on the unseen IDA-BD test split.
...
def train_phase1(args: argparse.Namespace, device: torch.device) -> Path:
    print("\n================ IDA-BD PHASE I: xBD -> IDA-BD LOCALIZATION FINE-TUNING ================", flush=True)
    ...
    if args.init_phase1_from:
        init_path = Path(args.init_phase1_from)
        if not init_path.exists():
            raise FileNotFoundError(f"--init-phase1-from not found: {init_path}")
        print(f"Initializing Phase I from xBD checkpoint: {init_path}", flush=True)
        ckpt_init = load_model_weights(model, init_path, device)
        print(
            f"Loaded xBD Phase I init: epoch={ckpt_init.get('epoch', 'NA')} "
            f"best_metric={ckpt_init.get('best_metric', 'NA')} "
            f"best_threshold={ckpt_init.get('best_threshold', 'NA')}",
            flush=True,
        )
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.phase1_lr, weight_decay=args.weight_decay, betas=(0.9, 0.999))
    ...
    for epoch in range(1, args.phase1_epochs + 1):
        ...
        if val_f1 > best_f1:
            ...
            save_checkpoint(
                ...
                best_f1,
                args,
                extra={"best_threshold": best_threshold, "dataset": "IDABD", "initialized_from_xbd": args.init_phase1_from},
            )
        ...
        if no_improve >= args.phase1_early_stopping_patience:
...
def train_phase2(args: argparse.Namespace, device: torch.device, phase1_ckpt: Path) -> Path:
    print("\n================ IDA-BD PHASE II: xBD -> IDA-BD DAMAGE FINE-TUNING ================", flush=True)
    ...
    if args.init_phase2_from:
        init_path = Path(args.init_phase2_from)
        if not init_path.exists():
            raise FileNotFoundError(f"--init-phase2-from not found: {init_path}")
        print(f"Initializing Phase II from xBD v5 checkpoint: {init_path}", flush=True)
        ckpt_init = load_model_weights(model, init_path, device)
        print(
            f"Loaded xBD Phase II init: epoch={ckpt_init.get('epoch', 'NA')} "
            f"best_metric={ckpt_init.get('best_metric', 'NA')}",
            flush=True,
        )
    else:
        print("No --init-phase2-from provided; initializing Phase II backbone from the fine-tuned Phase I checkpoint.", flush=True)
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.phase2_lr, weight_decay=args.weight_decay, betas=(0.9, 0.999))
    ...
    def validate_and_save(epoch_label: int, meters, phase_label: str):
        ...
        extra = {
            ...
            "dataset": "IDABD",
            "initialized_phase2_from_xbd": args.init_phase2_from,
        }
...
def evaluate_val_ablation_and_test(args: argparse.Namespace, device: torch.device, phase1_ckpt: Path, phase2_ckpt: Path) -> None:
    ...
    final = {
        "experiment": "xBD-to-IDA-BD supervised fine-tuning HRTBDA v5 multi-label rare-crop cascade",
        "phase1_checkpoint": str(phase1_ckpt),
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser("Fine-tune xBD-trained HRTBDA v5 on IDA-BD with 80/10/10 split")
    ...
    p.add_argument("--phase1-checkpoint", default="", help="Existing fine-tuned Phase-I checkpoint to use/resume/evaluate.")
    p.add_argument("--phase2-checkpoint", default="", help="Existing fine-tuned Phase-II checkpoint to use/evaluate.")
    p.add_argument("--init-phase1-from", default="", help="xBD-trained Phase-I checkpoint used to initialize IDA-BD Phase-I fine-tuning.")
    p.add_argument("--init-phase2-from", default="", help="xBD-trained v5 Phase-II checkpoint used to initialize IDA-BD Phase-II fine-tuning.")
    ...
    p.add_argument("--lr", type=float, default=2e-5, help="Deprecated fallback LR; use --phase1-lr and --phase2-lr.")
    p.add_argument("--phase1-lr", type=float, default=1e-5)
    p.add_argument("--phase2-lr", type=float, default=2e-5)
    ...
    p.add_argument("--phase1-early-stopping-patience", type=int, default=10)
...
def main() -> None:
    ...
    print("===== xBD -> IDA-BD HRTBDA V5 SUPERVISED FINE-TUNING =====", flush=True)
    ...
    print(f"xBD Phase-I init: {args.init_phase1_from or 'none'}", flush=True)
    print(f"xBD Phase-II init: {args.init_phase2_from or 'none'}", flush=True)
    print(f"Phase I fine-tune epochs: {args.phase1_epochs} | LR={args.phase1_lr}", flush=True)
    print(f"Phase II fine-tune epochs: {args.phase2_epochs} | LR={args.phase2_lr}", flush=True)
