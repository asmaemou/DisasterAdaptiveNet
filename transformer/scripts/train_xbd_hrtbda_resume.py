...
def train_phase1(args: argparse.Namespace, device: torch.device) -> Path:
    ...
    # Load old Phase I history so resuming preserves all previous rows and best F1.
    ...
    if history_path.exists():
        try:
            ...
            if history:
                ...
                print(
                    f"Loaded existing Phase I history: {len(history)} rows | "
    ...
    resume_value = getattr(args, "resume_phase1_from", "")
    ...
    if resume_path is not None:
        ...
        if ckpt.get("optimizer") is not None:
            ...
            # Move optimizer tensors to current device after loading.
        ...
        # phase1_last.pt stores the best validation metric/threshold seen so far.
        best_f1 = float(ckpt.get("best_metric", best_f1))
        best_threshold = float(ckpt.get("best_threshold", best_threshold))

        # Recover best_epoch from history if possible.
        if history:
            matching = [
                r for r in history
            ...
            if matching:
                ...
                best_epoch = int(max(history, key=lambda r: float(r.get("val_localization_f1", -1.0))).get("epoch", 0))
