...
DATASET_NAMES = ("xbd", "ida", "ian", "irma")
...
class XBDStyleLabeledDataset(BasePairDataset):
    def __init__(self, root: str | Path, split: str, image_size: int, training: bool, source_name: str, conditioning_id: int = 0):
...
class XBDStyleUnlabeledDataset(BasePairDataset):
    def __init__(self, root: str | Path, split: str, image_size: int, training: bool, source_name: str, conditioning_id: int = 0):
        super().__init__(image_size=image_size, training=training, conditioning_id=conditioning_id)
        self.root = Path(root)
        self.split_root = self.root / split
...
class CDANConditionedDomainDiscriminator(nn.Module):
    def __init__(self, feat_dim: int = 5, prob_dim: int = 5, hidden_dim: int = 128) -> None:
        super().__init__()
        in_dim = feat_dim * prob_dim
    ...
    def forward(self, pooled_logits: torch.Tensor, pooled_probs: torch.Tensor, grl_lambda: float) -> torch.Tensor:
        cond = torch.bmm(pooled_probs.unsqueeze(2), pooled_logits.unsqueeze(1)).flatten(1)
        cond = grad_reverse(cond, grl_lambda)
        return self.net(cond).squeeze(1)


def get_dataset_root_and_splits(args: argparse.Namespace, name: str) -> Tuple[Path, str, str, str]:
    if name == "xbd":
        return Path(args.xbd_root), args.xbd_train_split, args.xbd_val_split, args.xbd_test_split
    if name == "ida":
        return Path(args.ida_root), args.ida_train_split, args.ida_val_split, args.ida_test_split
    if name == "ian":
        return Path(args.ian_root), args.ian_train_split, args.ian_val_split, args.ian_test_split
    if name == "irma":
        return Path(args.irma_root), args.irma_train_split, args.irma_val_split, args.irma_test_split
    raise ValueError(f"Unsupported dataset: {name}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser("DisasterAdaptiveNet + CDAN with xBD/IDA/IAN/IRMA")
    ...
    parser.add_argument("--irma-train-split", type=str, default="train")
    parser.add_argument("--irma-val-split", type=str, default="val")
    parser.add_argument("--irma-test-split", type=str, default="test")
...
def pool_logits_and_probs(logits: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    pooled_logits = F.adaptive_avg_pool2d(logits, output_size=1).flatten(1)
    pooled_probs = F.softmax(pooled_logits, dim=1)
    return pooled_logits, pooled_probs
...
def main() -> None:
    ...
    source_root, source_train_split, source_val_split, _ = get_dataset_root_and_splits(args, args.source_dataset)
    target_root, target_train_split, target_val_split, target_test_split = get_dataset_root_and_splits(args, args.target_dataset)

    print(f"Loading source train: {args.source_dataset} ({source_train_split})...", flush=True)
    src_train = XBDStyleLabeledDataset(source_root, source_train_split, args.img_size, True, args.source_dataset, args.conditioning_id)
    print(f"Loading source val: {args.source_dataset} ({source_val_split})...", flush=True)
    src_val = XBDStyleLabeledDataset(source_root, source_val_split, args.img_size, False, args.source_dataset, args.conditioning_id)
    print(f"Loading target unlabeled train: {args.target_dataset} ({target_train_split})...", flush=True)
    tgt_train_u = XBDStyleUnlabeledDataset(target_root, target_train_split, args.img_size, True, args.target_dataset, args.conditioning_id)
    print(f"Loading target unlabeled val: {args.target_dataset} ({target_val_split})...", flush=True)
    tgt_val_u = XBDStyleUnlabeledDataset(target_root, target_val_split, args.img_size, True, args.target_dataset, args.conditioning_id)
    print(f"Loading target test: {args.target_dataset} ({target_test_split})...", flush=True)
    tgt_test = XBDStyleLabeledDataset(target_root, target_test_split, args.img_size, False, args.target_dataset, args.conditioning_id)
    ...
    print(f"Source {args.source_dataset} train samples: {len(src_train)} | source val samples: {len(src_val)}", flush=True)
    print(f"Target {args.target_dataset} unlabeled train samples: {len(tgt_train_u)} | unlabeled val samples: {len(tgt_val_u)}", flush=True)
    print(f"Target {args.target_dataset} test samples: {len(tgt_test)}", flush=True)
    ...
    domain_disc = CDANConditionedDomainDiscriminator(feat_dim=5, prob_dim=5, hidden_dim=128).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, src_batch in enumerate(iterator, start=1):
            ...
            if USE_TORCH_AMP:
                with autocast(device_type=device.type, enabled=args.amp and device.type == "cuda"):
                    ...
                    src_pooled_logits, src_pooled_probs = pool_logits_and_probs(src_logits)
                    tgt_pooled_logits, tgt_pooled_probs = pool_logits_and_probs(tgt_logits)
                    src_domain_logits = domain_disc(src_pooled_logits, src_pooled_probs, grl_lambda)
                    tgt_domain_logits = domain_disc(tgt_pooled_logits, tgt_pooled_probs, grl_lambda)
            ...
            if use_tqdm:
                iterator.set_postfix(
                    ...
                    domt=f"{domain_tgt_meter.avg:.4f}",
                )
                ...
                print(
                    ...
                    f"dom_src={domain_src_meter.avg:.4f} | dom_tgt={domain_tgt_meter.avg:.4f}",
                    flush=True,
                )
        ...
        row = {
            ...
            "train_domain_tgt": domain_tgt_meter.avg,
        ...
        if improved:
            ...
            print(
                f"No improvement for {epochs_without_improvement} epoch(s). Best epoch so far: {best_epoch} | best_score={best_score:.4f}",
                flush=True,
            )
