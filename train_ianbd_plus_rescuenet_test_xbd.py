# train_ianbd_plus_rescuenet_test_xbd.py
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser("Train on IAN-BD + RescueNet-xBD, test on xBD")
    parser.add_argument("--ianbd-root", type=str, default="/homes/j244s673/documents/wsu/phd/idabd_disasteradaptivenet")
    parser.add_argument("--rescuenet-root", type=str, default="/homes/j244s673/documents/wsu/phd/uda_two_stage/rescuenet_xbd")
    parser.add_argument("--target-root", type=str, default="/path/to/xbd")
    parser.add_argument("--ianbd-train-split", type=str, default="train")
    parser.add_argument("--ianbd-val-split", type=str, default="val")
    ...
    parser.add_argument("--output-dir", type=str, default="output/ianbd_plus_rescuenet_to_xbd")
...
def main() -> None:
    ...
    ianbd_train = XBDOriginalDataset(args.ianbd_root, args.ianbd_train_split, args.img_size, True, args.conditioning_id)

    print("Loading IAN-BD val...", flush=True)
    ianbd_val = XBDOriginalDataset(args.ianbd_root, args.ianbd_val_split, args.img_size, False, args.conditioning_id)
    ...
    train_loader = make_balanced_concat_loader([ianbd_train, rescuenet_train], args.batch_size, args.num_workers)
    val_loader = make_eval_loader(ConcatDataset([ianbd_val, rescuenet_val]), args.batch_size, args.num_workers)
    target_test_loader = make_eval_loader(target_test, args.batch_size, args.num_workers)

    loc_pos_weight, dmg_class_weights = aggregate_counts([ianbd_train, rescuenet_train])

    print(f"IAN-BD train samples: {len(ianbd_train)} | IAN-BD val samples: {len(ianbd_val)}", flush=True)
