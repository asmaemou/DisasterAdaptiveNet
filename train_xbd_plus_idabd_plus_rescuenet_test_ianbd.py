...
def set_perf_flags(device: torch.device) -> None:
    if device.type == "cuda":
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser("Train on xBD + IDA-BD + RescueNet-xBD, test on IAN-BD")
    ...
    parser.add_argument("--output-dir", type=str, default="output/xbd_plus_idabd_plus_rescuenet_to_ianbd")
...
def make_model(device: torch.device, do_compile: bool) -> nn.Module:
    cfg = SimpleNamespace(
    ...
    if do_compile and hasattr(torch, "compile"):
        try:
            ...
            print(f"torch.compile could not be enabled: {e}", flush=True)
...
def make_balanced_concat_loader(
...
def evaluate_target_test_f1(
    ...
    for batch in iterator:
        batch = move_batch_to_device(batch, device)
        img = batch["img"]
        loc_true = batch["loc"].long()
        dmg_true_raw = batch["dmg"].long()
        cond_id = batch["cond_id"]
...
def main() -> None:
    ...
    print("Loading xBD train...", flush=True)
    xbd_train = XBDOriginalDataset(args.xbd_root, args.xbd_train_split, args.img_size, True, args.conditioning_id)

    print("Loading xBD val...", flush=True)
    xbd_val = XBDOriginalDataset(args.xbd_root, args.xbd_val_split, args.img_size, False, args.conditioning_id)

    print("Loading IDA-BD train...", flush=True)
    ida_train = XBDOriginalDataset(args.ida_root, args.ida_train_split, args.img_size, True, args.conditioning_id)
    ...
    print("Loading IAN-BD test...", flush=True)
    ...
    train_loader = make_balanced_concat_loader(
        [xbd_train, ida_train, rescuenet_train],
        args.batch_size,
        args.num_workers,
        args.prefetch_factor,
    )
    val_loader = make_eval_loader(
        ConcatDataset([xbd_val, ida_val, rescuenet_val]),
        args.batch_size,
        args.num_workers,
        args.prefetch_factor,
    )
    target_test_loader = make_eval_loader(
        target_test,
        args.batch_size,
        args.num_workers,
        args.prefetch_factor,
    ...
    loc_pos_weight, dmg_class_weights = aggregate_counts([xbd_train, ida_train, rescuenet_train])
    ...
    model = make_model(device, args.compile)
