...
def apply_shared_aug(image: np.ndarray, mask: Optional[np.ndarray], training: bool):
    # Intentionally left as identity to match the user's request of no augmentation.
    return image, mask
...
class ResNet34UNetLocalization(nn.Module):
    def __init__(self, pretrained: bool = True) -> None:
        ...
        if pretrained:
            ...
            print("using weights from ResNet34_Weights.IMAGENET1K_V1", flush=True)
        else:
            backbone = resnet34(weights=None)
            print("using random initialization for ResNet34 backbone", flush=True)
        ...
        self.dec1 = self._decoder_block(64 + 64, 32)
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        s1 = self.stem(x)                # 64, H/2
        x = self.maxpool(s1)            # H/4
        e1 = self.layer1(x)             # 64, H/4
        e2 = self.layer2(e1)            # 128, H/8
        e3 = self.layer3(e2)            # 256, H/16
        e4 = self.layer4(e3)            # 512, H/32

        x = torch.nn.functional.interpolate(e4, size=e3.shape[-2:], mode="bilinear", align_corners=False)
        x = torch.cat([x, e3], dim=1)
        x = self.dec4(x)

        x = torch.nn.functional.interpolate(x, size=e2.shape[-2:], mode="bilinear", align_corners=False)
        x = torch.cat([x, e2], dim=1)
        x = self.dec3(x)

        x = torch.nn.functional.interpolate(x, size=e1.shape[-2:], mode="bilinear", align_corners=False)
        x = torch.cat([x, e1], dim=1)
        x = self.dec2(x)

        x = torch.nn.functional.interpolate(x, size=s1.shape[-2:], mode="bilinear", align_corners=False)
        x = torch.cat([x, s1], dim=1)
        x = self.dec1(x)

        logits = self.final_head(x)
        logits = torch.nn.functional.interpolate(logits, scale_factor=2.0, mode="bilinear", align_corners=False)
        return logits[:, 0], e4


class LocalizationDomainDiscriminator(nn.Module):
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser("Stage 1 localization with ResNet34 U-Net + DANN")
...
def evaluate_localization(model: nn.Module, loader: DataLoader, criterion: BCEDiceLoss, device: torch.device, args: argparse.Namespace):
    ...
    for batch in iterator:
        ...
        logits, _ = model(img)
        loc_bce, loc_dice_loss = criterion(logits, loc)
        loss = args.loc_bce_weight * loc_bce + args.loc_dice_weight * loc_dice_loss
        ...
        bce_meter.update(loc_bce.item(), bs)
...
def main() -> None:
    ...
    print("Stage-1 uses a single pre-disaster RGB image as input for ResNet34 U-Net.", flush=True)
    ...
    model = ResNet34UNetLocalization(pretrained=not args.no_pretrained).to(device)
    domain_disc = LocalizationDomainDiscriminator(512).to(device)
        ...
        for step, src_batch in enumerate(iterator, start=1):
            ...
            if not torch.isfinite(total_loss):
                print(
                    ...
                    f"sup_loss={float(sup_loss):.6f}, dom_loss={float(dom_loss):.6f}. Skipping step.",
                    flush=True,
                )
                optimizer.zero_grad(set_to_none=True)
                continue
