...
class BaseDamageDataset(Dataset):
    ...
    def _build_damage_target_from_standard_mask(loc: np.ndarray, dmg: np.ndarray) -> np.ndarray:
        ...
        Output target convention for damage CE:
          0 no damage
          1 minor damage
          2 major damage
          3 destroyed
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser("Train on IDA-BD + IAN-BD, test on xBD")
    ...
    parser.add_argument("--output-dir", type=str, default="output/idabd_plus_ianbd_to_xbd")
