...
class BaseLocalizationDataset(Dataset):
    ...
    def _normalize_prepre(self, pre: np.ndarray) -> np.ndarray:
        x = np.concatenate([pre.astype(np.float32) / 255.0, pre.astype(np.float32) / 255.0], axis=2)
        x = x.transpose(2, 0, 1)
    ...
    def _finalize_labeled(self, image: np.ndarray, loc: np.ndarray, stem: str):
        ...
        return {
            "img": torch.from_numpy(self._normalize_prepre(image)).float(),
...
class LocalizationCDANDiscriminator(nn.Module):
    """
    Practical CDAN-style discriminator for Stage 1.
    Conditions domain prediction on pooled localization logits + pooled probabilities.
    """
    def __init__(self) -> None:
        super().__init__()
    ...
    def forward(self, logits: torch.Tensor, grl_lambda: float) -> torch.Tensor:
        pooled_logits = torch.mean(logits, dim=(2, 3))
        pooled_probs = torch.mean(torch.sigmoid(logits), dim=(2, 3))
        cond = torch.cat([pooled_logits, pooled_probs], dim=1)
        cond = grad_reverse(cond, grl_lambda)
        return self.net(cond).squeeze(1)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser("Stage 1 localization with CDAN-style domain adaptation")
    ...
    parser.add_argument("--domain-weight", type=float, default=0.03)
...
def main() -> None:
    ...
    print("Stage-1 CDAN uses [pre, pre] as a 6-channel paired input for DisasterAdaptiveNet.", flush=True)
    ...
    domain_disc = LocalizationCDANDiscriminator().to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, src_batch in enumerate(iterator, start=1):
            ...
            src_cond = torch.full((src_img.size(0), 1), args.conditioning_id, dtype=torch.long, device=device)
            tgt_cond = torch.full((tgt_img.size(0), 1), args.conditioning_id, dtype=torch.long, device=device)
            ...
            with ctx:
                src_logits = model(src_img, src_cond)[:, 0:1]
                tgt_logits = model(tgt_img, tgt_cond)[:, 0:1]
                loc_bce, loc_dice = criterion(src_logits[:, 0], src_loc)
                sup_loss = args.loc_bce_weight * loc_bce + args.loc_dice_weight * loc_dice

                src_dom_logits = domain_disc(src_logits, grl_lambda)
                tgt_dom_logits = domain_disc(tgt_logits, grl_lambda)
            ...
            if not torch.isfinite(total_loss):
                print(f"Skipping non-finite loss at epoch={epoch} step={step}", flush=True)
                continue
